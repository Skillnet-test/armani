...
public class CMSContainer extends UnicastRemoteObject implements ICMSContainer,
		...
		try {
			...
			if (con == null) {
				throw new NullPointerException(
						"no constructor found for ".concat(String
								.valueOf(String.valueOf(cls.getName()))));
			}
			Object obj = con
					.newInstance(new Object[] { tuning.getProperties() });
			...
			System.out.println("CMSContainer.createCMSComponent()->"
					.concat(String.valueOf(String.valueOf(itx
							.getTargetException()))));
	...
	public boolean hasErrorBeenLogged() {
		try {
			...
			if (!file.exists()) {
				...
				System.out
						.println("CMSContainer.hasErrorBeenLogged() file.length: "
								.concat(String.valueOf(String.valueOf(file
										.length()))));
				boolean flag1 = file.length() > (long) 0;
				return flag1;
			}
		} catch (Exception ex) {
			System.out.println("Exception CMSContainer.hasErrorBeenLogged()->"
					.concat(String.valueOf(String.valueOf(ex))));
		}
		boolean flag2 = true;
		return flag2;
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.err.println("Exception CMSContainer.main()->".concat(String
					.valueOf(String.valueOf(ex))));
			LoggingServices.getCurrent()
					.logMsg("CMSContainer",
							"main",
	...
	public boolean requestComponentShutdown(String componentName, String uid)
			throws RemoteException {
	...
	public void requestContainerShutdown() throws RemoteException {
		...
		if (isShutdownInProgress) {
			System.out
					.println("CMSContainer.requestContainerShutdown returning because (isShutdownInProgress == true)");
			return;
		}
		isShutdownInProgress = true;
		String[] compNames = getComponents();
		System.out
				.println("CMSContainer.requestContainerShutdown()-> compNames.length: ".concat(String
						.valueOf(String.valueOf(compNames.length))));
		...
		System.out.println("CMSContainer.requestContainerShutdown()->"
				.concat(String.valueOf(String.valueOf(getUID()))));
	...
	public void saveContainerVitalsHolder(ContainerVitalsHolder holder) {
		try {
			FileWriter writer = new FileWriter(FileMgr.getLocalFile(
					"container", String.valueOf(String
							.valueOf((new StringBuffer(String.valueOf(String
									.valueOf(getContainerName())))).append("_")
									.append(getUID()).append(".xml")))));
			...
			System.out.println("Exception saveContainerVitalsHolder()->"
					.concat(String.valueOf(String.valueOf(ex))));
		}
	}
	...
	public void updateContainerTuning(ContainerTuning containerTuning) {
		...
		for (int x = 0; x < compTunings.length; x++) {
			if (!htComps.containsKey(compTunings[x].getName())) {
				System.out.println("updateContainerTuning()-> new component:"
						.concat(String.valueOf(String.valueOf(compTunings[x]
								.getName()))));
		...
		label0: do {
			...
			System.out.println("updateContainerTuning()-> removing component:"
					.concat(String.valueOf(String.valueOf(key))));
			htComps.remove(key);
		} while (true);
	}
	...
	protected void stopComponent(ICMSComponent theComponent) {
		...
		try {
			...
			System.out.println("CMSContainer.stopComponent()->".concat(String
					.valueOf(String.valueOf(compName))));
		} catch (RemoteException ex) {
			System.out.println("CMSContainer.stopComponent()-> ".concat(String
					.valueOf(String.valueOf(ex))));
			System.out
					.println("CMSContainer.stopComponent()-> cannot remove component from reference without name");
		}
		try {
			NamingService
					.unbind(String.valueOf(getRNSLocation())
							+ String.valueOf(compName), theComponent);
			System.out
					.println("CMSContainer.stopComponent()-> component unbound");
	...
	boolean requestComponentShutdown(String componentName, int reason)
			throws RemoteException {
	...
	boolean shutdownComponent(String componentName, String uid, int reason) {
		try {
			...
			System.out.println(String.valueOf(String.valueOf((new StringBuffer(
					"requestComponentShutdown()->")).append(componentName)
					.append(":").append(uid))));
			ICMSComponent[] comps = getCMSComponents(componentName);
			if (comps == null) {
				boolean flag = false;
			...
			System.out.println("Exception stopComponent()->".concat(String
					.valueOf(String.valueOf(ex))));
		}
		boolean flag2 = false;
		return flag2;
	...
	private void allocNewVitalsHistory() {
		try {
			...
			ContainerVitalsHolder holder = new ContainerVitalsHolder(
					getContainerName(), getUID(), tuning.getHoursOfHistory());
			saveContainerVitalsHolder(holder);
		} catch (Exception ex) {
			System.out.println("Exception allocNewVitalsHistory()->"
					.concat(String.valueOf(String.valueOf(ex))));
		}
	}
	...
	private void bind() {
		try {
			String bindName = String.valueOf(getRNSLocation())
					+ String.valueOf(containerName);
			System.out.println("Binding to: ".concat(String.valueOf(String
					.valueOf(bindName))));
			NamingService.bind(bindName, this);
		} catch (Exception e) {
			System.out
					.println("ERROR Unable to bind to local Naming Service()->"
