...
public class LineItemPOSUtil extends BusinessObject {
        ...
        for (Enumeration enm = lnItmDtlArr[i].getReductions(); enm.hasMoreElements(); ) {
          ...
          if (reason.indexOf(" discount") > 0) {
            if (reduction instanceof CMSReduction) {
              ...
              if (type.toLowerCase().equals(cmsReduction.getDiscount().getType().toLowerCase())) {
  ...
  /**
   *on 22-01-2017-For getting total discount reason base
  * By saptarshi
  * @return Map
  */
 public Map getReductionPercentageByReason() {
   Map reductionDisMap = new HashMap();
   Discount dis = null;
   String reason = "";   
   double percent=0.0;
   POSLineItemDetail[] lnItmDtlArr = this.posLineItem.getLineItemDetailsArray();
   if (lnItmDtlArr != null && lnItmDtlArr.length > 0) {
     for (int i = 0; i < lnItmDtlArr.length; i++) {
       for (Enumeration enm = lnItmDtlArr[i].getLineItem().getDiscounts(); enm.hasMoreElements(); ) {
    	   
         dis = (Discount)enm.nextElement();         
         reason = dis.getReason();   
         //if u want just adding of percentage you will get this from here but 
         // if u want calculation then set the formula here.
         if (reductionDisMap.containsKey(reason)) {        	 
        	 try {        		 
        		 percent=((Double)reductionDisMap.get(reason)).doubleValue();
        		 percent+=dis.getPercent();
        		 reductionDisMap.put(reason,percent); 
               } catch (Exception e) {}
             
           } else {        	 
          	 reductionDisMap.put(reason, dis.getPercent());
           }        
         
       }
     }
   }
   return reductionDisMap;
 }
 
 /**
  *on 08-02-2017-For getting total discount reason base
 * By saptarshi
 * @return Map
 */
public Map getReductionPercentageByReason1() {
  Map reductionDisMap = new HashMap();
  Discount dis = null;
  String reason = "";   
  double percent=0.0;
