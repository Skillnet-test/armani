 ...
 //Modified by deepika to handle NullPointerException for promotion code
  public IPromotion getPromotionById(String anId) {
	...
	try {
		 ...
		 if(promotion==null){ // added by saptarshi
			 throw new NullPointerException();
		 }
	} catch (Exception e) {
		// TODO Auto-generated catch block
		System.out.println("Promotions has not been loaded in xml:-"+anId);
		//e.printStackTrace();
	}
	return promotion;
  }
				...
				//start-default code for where item does not have any promotion
				  ...
				  if(countPromtion.size() > 0){
				      ...
				      while(promotionId.hasNext()){
				    	  ...
				    	  try{
				    	 if (this.isOnPromotionTimeRange(promoItem)==true){ // commented by saptarshi
				    	  ...
				    	  while(rankedActivatedPromotions.hasNext()) {
				    		  activatedPromotion = (IActivatedPromotion)rankedActivatedPromotions.next();
			               	  ...
			               	  Double markdown=0.0;
			               	  if(promoItem!=null && promoItem.getId()!=null && promoid!=null){ // null pointer exception check by saptarshi
