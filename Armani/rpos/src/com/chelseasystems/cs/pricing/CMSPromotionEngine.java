...
*/
  ...
  public static POSLineItemDetail[] sortByAmount(POSLineItemDetail[] details) {
	  //try by saptarshi
	  boolean returnOnCodition=false;
	  POSLineItemDetail[] lineItemArr=CMSPromotionBasedPriceEngine.getLineItemArray();
	  List countPromtion;
	  for(int k=0;k<lineItemArr.length;k++)
      {
    	  countPromtion=new ArrayList();
    	  Iterator promotionIdForCount=lineItemArr[k].getLineItem().getItem().getPromotionIds();
    	  while(promotionIdForCount.hasNext()){
			  countPromtion.add(promotionIdForCount.next());
		  }
    	  if(countPromtion.size()>1)
    	  {
    		  returnOnCodition=true;
    		  break;
    	  }
      }
	  if(returnOnCodition==true){
