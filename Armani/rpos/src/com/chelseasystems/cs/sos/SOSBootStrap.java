    ...
    try {
      ...
      //Store object does not exist locally and EU needs to validate new store
       ...
       //Poonam: Added for Expiry_DATE issue on Nov 22,2016 
    	  ...
    	  if("US".equalsIgnoreCase(Version.CURRENT_REGION) && currStore==null){
          	tempStore = CMSStoreHelper.findByStoreId(theMgr,tempStoreId ,(java.sql.Date)currProcessDate);
          }else if("US".equalsIgnoreCase(Version.CURRENT_REGION)){
        	  tempStore = CMSStoreHelper.findByStoreId(theMgr, currStore.getId(),(java.sql.Date)currProcessDate);
            }
          else{
