	...
	public ParametricStatement[] getInsertSQL(CompositePOSTransaction object, boolean subtract) {
		...
		if (txnType.equals("SALE")) {
			...
			for( int i=0;i<len; i++){
				...
				else if(array[i].getDiscount()==null){ //saptarshi-Have to change cos have to set transaction discount
					//custSaleSummary.setDisPercent(0.0d);
					POSLineItemDetail[] posLineDetailArr=array[i].getLineItemDetailsArray();
					Reduction reduction = null;
					ArmCurrency disPer=null;
					boolean IsNoTransDis=false;
					if (posLineDetailArr != null && posLineDetailArr.length > 0) {
						for (int j = 0; j < posLineDetailArr.length; j++) {
							for (Enumeration enm = posLineDetailArr[j].getReductions(); enm.hasMoreElements(); ) {
								reduction = (Reduction)enm.nextElement();
    							ArmCurrency reducAmt=reduction.getAmount();
    							if(reducAmt!=null){
    								Double extendedRetailAmount=array[i].getExtendedRetailAmount().doubleValue();
        							disPer=reducAmt.divide(extendedRetailAmount);
        							custSaleSummary.setDisPercent(disPer.doubleValue());
        							//Double discountPercent=disPer.multiply(100).doubleValue();	
    							}
						...
						if (posLineDetailArr != null && posLineDetailArr.length > 0) {
							for (int j = 0; j < posLineDetailArr.length; j++) {
								for (Enumeration enm = posLineDetailArr[j].getReductions(); enm.hasMoreElements(); ) {
	    							...
	    							ArmCurrency reducAmt=reduction.getAmount();
	    							if(reducAmt!=null){
	    								Double extendedRetailAmountRetn=array[i].getExtendedRetailAmount().doubleValue();
	        							disPer=reducAmt.divide(extendedRetailAmountRetn);
	        							custSaleSummary.setDisPercent(disPer.doubleValue());
	        							//Double discountPercent=disPer.multiply(100).doubleValue();	
	    							}
