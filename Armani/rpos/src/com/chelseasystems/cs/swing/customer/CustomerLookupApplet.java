...
//	1922
	...
	/*private void enterSocialSecurityNo() {
		theAppMgr.setSingleEditArea(res.getString("Enter social security number"), "CUST_SEARCH_BY_SOCIALSECURITYNO");
	}*/
	...
	public void editAreaEvent(String sCommand, String sInput) {
		...
		/*}else if (sCommand.equals("CUST_SEARCH_BY_SOCIALSECURITYNO")) { // added by saptarshi
			if (sInput == null || sInput.trim().length() < 1 || sInput.trim().length() > 20) {
				theAppMgr.showErrorDlg(res.getString("Social Security Number not valid"));
				enterSocialSecurityNo();
				return;
			} else {
				findBySocialSecurityNo(sInput);
			}*/
...
//			}
			...
			enterCustPhone();
		} /*else if (sAction.equals("CUST_SEARCH_BY_SOCIALSECURITYNO")) {
			enterSocialSecurityNo();
	...
	// added by saptarshi for search for social security no
	/*private void findBySocialSecurityNo(String sCustSocialSecNo) {
		
		try {
			CMSCustomer cmsCustomer = (CMSCustomer) CMSCustomerHelper.findBySocialSecurityNo(theAppMgr, sCustSocialSecNo);
			if (cmsCustomer == null || cmsCustomer.getCustomerType() == null) {
				showNoCustFound();
				return;
			}
			enter();
...
//	 issue # 1922 
	...
	private void processQuery(String sQuery) {
		try {
			...
			// end VISHAL : added for search criteria in EUR franchising STORE 16 sept 2016
			if (cmsCustomer == null || cmsCustomer.length < 1) {
				showNoCustFound();
				setFocusOnLastName();
...
//		issue 1922 add doc number label
	...
	public boolean isHomeAllowed() {
		...
		return (theAppMgr.showOptionDlg(res.getString("Cancel Transaction"), res
	...
	//Sonali: added for DiscountCode=VIP_DISCOUNT.CODE
		...
		//Insert Into Arm_Cfg_Detail (Ty_Cfg,Cd_Cfg,Ed_Co,Ed_La,De_Cfg,Priority)Values('VIP_DISCOUNT','87','CN','zh','Vip','1'); 
