...
public class CustomerManagementApplet extends CMSApplet {
	...
	public String getVersion() {
		return ("$Revision: 1.78.2.12.4.35 $");
	}
...
/*	private void checkCardToken(String string) {
	...
	public void appButtonEvent(CMSActionEvent anEvent) {
		...
		// added by Anand on 04/12/2005. Launch CRM when the button pressed is LAUNCH_CRM.
			...
			// checking for social security number
			 // start -change by saptarshi mukhaty
			 if((pnlMisc.getSocialSecurityNo() == null || pnlMisc.getSocialSecurityNo().trim().length() < 1)){
					theAppMgr.showErrorDlg(res.getString("Please fill in social security no"));
					//pnlMisc.getSocialSecurityNoTxt().requestFocus();
					anEvent.consume();
					return;
				}
			 // end-change by saptarshi mukhaty
	...
	private void loadCustomer() {
		try {
			...
			//start- changed by saptarshi
			pnlMisc.setSocialSecurityNo(cmsCustomer.getSocialSecurityNo());			
			// end- changed by saptarshi
			//Added for Privacy Mgmt Marketing and Master
			if("EUR".equalsIgnoreCase(Version.CURRENT_REGION)){
	...
	private boolean createModifyCustomer() {
		try {
			...
			//start- changed by saptarshi
			if (pnlMisc.getSocialSecurityNo() != null)
				cmsCustomer.setSocialSecurityNo(pnlMisc.getSocialSecurityNo());		    
			//end-- changed by saptarshi 			
