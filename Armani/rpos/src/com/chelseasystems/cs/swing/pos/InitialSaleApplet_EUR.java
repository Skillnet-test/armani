...
/*
 ...
 +------+------------+-----------+-----------+----------------------------------------------+
 | 29   | 11-14-2005 | Manpreet  | N/A       | initReservationsOpenTxn()->reservation.cfg is|
 |      |            |           |           | present, buildDepositItem()->cmsItem !=null  |
 +------+------------+-----------+-----------+----------------------------------------------+
 ...
 +------+------------+-----------+-----------+----------------------------------------------+
 | 25   | 08-11-2005 | Vikram    | 861       | Initiate loyalty when customer is ed to   |
 |      |            |           |           | transaction.                                 |
 +------+------------+-----------+-----------+----------------------------------------------+
...
//import msgsentry.msglog.Logger;
...
public class InitialSaleApplet_EUR extends CMSApplet {
	...
	private CMSRedeemableBalanceAppModel appModel; //added by Himani for GC Balance Inquiry 
	...
	public void start() {
		try {
			...
			// trying to move data from a transaction view to sale transaction.. starts
			if (theAppMgr.getStateObject("CREATE_SALE") != null || theAppMgr.getStateObject("ADD_TO_FISCAL") != null) {
			...
			pnlLineItem.clear();
			// trying to move data from a transaction view to sale transaction.. ends
			// check for customer
			if (theAppMgr.getStateObject("TXN_CUSTOMER") != null) {
				try {
					...
					if (theTxn != null && txnCust != null) {
						...
						// VATUtilities.applyVAT(theAppMgr, theTxn, (CMSStore)theTxn.getStore()
			...
			initHeaders();
			//added by shushma for duty free management
			String flag = (String)theAppMgr.getStateObject("ARM_BOARDING_PASS");
			if(!(flag==null)){
			...
			if ((isNoSaleMode() || isNoReturnMode()) && (theTxn.getCurrFiscalDocument() != null || theTxn.getId().length() > 0)) {
				String str = (String) theAppMgr.getStateObject("ARM_DIRECT_TO");
				if ((str == null) || (str.equals("SALE_APPLET") && theTxn.getCustomer() != null)) {
					showPrintFiscalMenu();
				}
			}
		} catch (Exception ex) {
		...
		if (theAppMgr.getStateObject("TAX_EXEMPT") != null) {
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					SwingUtilities.invokeLater(new Runnable() {
							public void run() {
							InitialSaleApplet_EUR.this.appButtonEvent(new CMSActionEvent(this, 0, (String) theAppMgr.getStateObject("TAX_EXEMPT"), 0));
	...
	public void swipeBoardingPass(){
		String builder = new ConfigMgr("airport.cfg").getString("BOARDING_PASS.BUILDER");
	...
	public void convertOldTxntoCurrent() {
		try {
			if (theAppMgr.getStateObject("CREATE_SALE") != null) {
				...
				CMSCompositePOSTransaction oldTxn = (CMSCompositePOSTransaction) theAppMgr.getStateObject("THE_TXN");
			...
			if (theAppMgr.getStateObject("ADD_TO_FISCAL") != null) {
				...
				ArrayList txns = (ArrayList) theAppMgr.getStateObject("TXNS_LOADING");
				for (int i = 0; i < txns.size(); i++) {
					CMSCompositePOSTransaction oldTxn = (CMSCompositePOSTransaction) txns.get(i);
					theTxn.convertSaleToNonFiscalTransaction(oldTxn, i);
				}

				/*
				 * if (oldTxn.getCustomer() != null) { theTxn.setCustomer(oldTxn.getCustomer()); theAppMgr.addStateObject("TXN_CUSTOMER",
				 * oldTxn.getCustomer()); }
				 */
				theAppMgr.addStateObject("TXN_POS", theTxn);
			}
	...
	public void stop() {
		if (!totalPressed) {
			theTxn.getAppModel(CMSAppModelFactory.getInstance(), theAppMgr).printCancelReceipt(theAppMgr);
		}
	...
	public void editAreaEvent(String Command, ArmCurrency amount) {
		...
		try {
			if (Command.equals("ADJUST_DEPOSIT") && (isReservationCloseMode() || isNoOpenReservationCloseSaleMode() || this.isNoOpenReservationCloseReturnMode())) {
				...
				try {
					// if (amount.greaterThan(theTxn.getCompositeTotalAmountDue().absoluteValue().
					// add(lineItmDeposit.getExtendedRetailAmount())
					//
					// ))
					if (!isValidDepositAmount(amount)) {
						theAppMgr.showErrorDlg(res.getString("Deposit amount can not be more than zero total amount due"));
						enterAdjustDepositAmount();
						return;
					}
				} catch (Exception e) {
			...
			if (Command.equals("MARKDOWN_AMT")) {
				...
				if (solicitOverrideReason) {
					...
					if (overRideDlg.isOK()) {
						...
						pnlLineItem.getSelectedLineItem().setOverrideAmount(amount);
	...
	public void editAreaEvent(String Command, String sEdit) {
		...
		try {
			...
			if (Command.equals("MANAGER")) {
				...
				try {
					//------ Fix for Issue#1880(Suspend Message) ---- Start-----------------
					//MB:11/20/2007 
					...
					//-- End Code Comment
						isGoHomeAllowed = theAppMgr.showOptionDlg(res.getString("Suspend Transaction"), res.getString("Are you sure you want to Suspend this transaction?"));
						if(isGoHomeAllowed)
						{
							ParkServices.suspend(theTxn, sEdit);
							theAppMgr.goHome();
				...
				/*
				 * Item[] itemSearch = ItemHelper.getItemByDivAndDesc(theAppMgr, itemDivision, sEdit); if (itemSearch == null) {
				 * theAppMgr.showErrorDlg("No items were found with that brand name in that division"); enterItem(); showButtons(); return; }
				 * ItemLookupDlg dlg = new ItemLookupDlg(theAppMgr.getParentFrame(), theAppMgr, itemSearch); dlg.setVisible(true); Item itemLookup =
				 * (Item)theAppMgr.getStateObject("ITEM_SELECT"); theAppMgr.removeStateObject("ITEM_SELECT"); //if canceled if (itemLookup == null) {
				 * enterItem(); showButtons(); return; } enterItem(itemLookup.getId() + "80000"); showButtons();
				 */
	...
	public void objectEvent(String Command, Object obj) {
		...
		/*
		 * I commented these to speed Item-lookup up. These gui calls were costing more than a second. We need to determine if there is a better way
		 * to ensure the gui is reset here. - CG 6/9/2002 showButtons(); enterItem();
		 */
		enterItem();
			...
			try {
				...
				for (int index = 0; index < discounts.length; index++) {
					...
					if (discount instanceof ArmLineDiscount) {
						theAppMgr.showErrorDlg(res.getString("By Amount Discount has been removed due to modification to Tax Exempt"));
					...
					try {
						...
						switch (iAppletMode) {
							...
							case NO_OPEN_RESERVATIONS_CLOSE_SALE:
								lineItem = theTxn.addNoReservationLineItemAsSale(item);
								break;
							case NO_OPEN_RESERVATIONS_CLOSE_RETURN:
								lineItem = theTxn.addNoReservationLineItemAsReturn(item);
								break;
							case NO_SALE_MODE:
						...
						lineItem.setExtendedStagingBarCode(itemWrapper.getExtendedStagingBarCode());
						}
			...
			try {
				...
				for(int i=0;i<lineItems.length;i++){
					if((lineItems[i].getOverrideAmount()!=null)){
						...
						lineItems[i].setManualUnitPrice(lineItems[i].getOverrideAmount());
			...
			if (obj != null) {
				...
				try {
					...
					if (((CMSItem) item.getItem()).isDeposit() && depositType != null && depositType.equals("CLOSE_DEPOSIT")) {
						...
						theAppMgr.showErrorDlg(res.getString("Please verify the Customer's deposit history"));
					} else {
						switch (iAppletMode) {
							case LAYAWAY_MODE:
							...
							case PRE_SALE_OPEN:
								...
								break;
							case RESERVATIONS_OPEN:
								// Def : 1603 : Reservatoion of Service Item: if the item is not deposit it shld be added as reservation service item
								// or sale item
		...
		if (Command.equals("BOARDING_PASS")) {
			...
			CMSCustomerBoardingPassDlg_EUR boardingPassDlg = new CMSCustomerBoardingPassDlg_EUR(theAppMgr, airportDetails);
			theAppMgr.setSingleEditArea(res.getString("Scan or Enter Manually Boarding Pass Details and press 'Enter'"));
	...
	public void appButtonEvent(String Header, CMSActionEvent anEvent) {
		...
		if (Header.equals("PRINT_FISCAL_NOSALE") && sAction.equals("OK")) {
			anEvent.consume();
			if (theTxn.getLineItemsArray() == null || theTxn.getLineItemsArray().length < 1 || (!hasLineItemsToPrintFiscalDocument())) {
				isGoHomeAllowed = true;
			} else {
				isGoHomeAllowed = (theAppMgr.showOptionDlg(res.getString("Finish Fiscal Printing"), res.getString("Are you sure you want to finish printing?")));
			}
			if (!isGoHomeAllowed)
				return;
			theAppMgr.goHome();
		...
		if (sAction.equals("DDT") || sAction.equals("TAX_FREE") || sAction.equals("VAT_INVOICE") || sAction.equals("CREDIT_NOTE")) {
			if (!hasLineItemsToPrintFiscalDocument()) {
				theAppMgr.showErrorDlg(res.getString("Fiscal document for all the itmes have been printed for this transaction."));
				anEvent.consume();
				return;
			}
			...
			if (sAction.equals("BACK")) {
				...
				updateDiscountLabel();
				if (isLayawayMode()) {
					//System.out.println("InitialSaleApplet_EUR.appButtonEvent(REMOVE)->theTxn.resetLayawayDeposit();"); // theTxn.resetLayawayDeposit();
				}
				pnlLineItem.repaint();
				updateLabel();
				showButtons();
					...
					try {
						...
						for (int i = 0; posLineItems != null && i < posLineItems.length; i++) {
							...
							if (posLineItems[i] instanceof CMSNoSaleLineItem)
								((CMSNoSaleLineItem) posLineItems[i]).removeAddPriceDiscount();
							if (posLineItems[i] instanceof CMSNoReturnLineItem)
								((CMSNoReturnLineItem) posLineItems[i]).removeAddPriceDiscount();
						}
						theTxn.removeAllDiscounts();
						theTxn.removeAllSettlementDiscounts();
						theAppMgr.removeStateObject("ARM_DISCOUNT_SEQNUM");
		...
		// Added transaction level discount
			...
			if (sAction.equals("CANCEL")) {
				theAppMgr.unRegisterSingleEditArea();
				theAppMgr.addStateObject("FROM_CANCEL_TAX_EXEMPT", "FROM_CANCEL_TAX_EXEMPT");
				goPrevious(anEvent);
	...
	private void showPopupGC(Redeemable redeemable)
	{
		...
		String curBal=null;
		   CMSStoreValueCard cmsstorevaluecard=(CMSStoreValueCard)redeemable;
	          ...
	          if(issueStoreId!=null && !issueStoreId.substring(0, 3).equalsIgnoreCase(currentStoreId.substring(0, 3))){
	        	  theAppMgr.showErrorDlg(res.getString("This gift card is issued by a different company"));
	        	  return;
	          }

		curBal=appModel.getRemainingBalance().formattedStringValue();
		
		theAppMgr.showErrorDlg(res.getString("GiftCard ID: "+ appModel.getRedeemableID()+"                "+"Current Balance: "+ curBal+"                "+"Issued Date: "+ issuedDate));
	...
	ArmCurrency depositBalance = null;
		...
		if (depositBalance == null)
			depositBalance = new ArmCurrency(0.0d);
		String sPrompt = res.getString("Enter new deposit amount") + " ( " + res.getString("Previous Balance") + depositBalance.formattedStringValue() + " )";
		theAppMgr.setSingleEditArea(sPrompt, "ADJUST_DEPOSIT", theAppMgr.CURRENCY_MASK);
	}
	...
	/**
	 * NOTE: IF THIS METHOD IS MODIFIED, DO SAME IN PAYMENT APPLET. AND VICE-VERSA (MSB - 06/23/05) Modify DDT Number
	 * 
	 * @param sInput
	 *            String
	 */
	private void modifyDDTNumber(String sInput) {
		try {
			if (!fiscalDocUtil.setNextDDTNumber(sInput)) {
				theAppMgr.showErrorDlg(res.getString("DDT number already used, please try another"));
				selectModifyDDT();
				return;
			}
	...
	/**
	 * NOTE: IF THIS METHOD IS MODIFIED, DO SAME IN PAYMENT APPLET. AND VICE-VERSA (MSB - 06/23/05) Modify TaxNumber
	 * 
	 * @param sInput
	 *            String
	 */
	private void modifyTAXNumber(String sInput) {
		try {
			if (!fiscalDocUtil.setNextVATNumber(sInput)) {
				theAppMgr.showErrorDlg(res.getString("Tax number already used, please try another"));
				selectModifyTaxNumber();
				return;
			}
	...
	/**
	 * NOTE: IF THIS METHOD IS MODIFIED, DO SAME IN PAYMENT APPLET. AND VICE-VERSA (MSB - 06/23/05) Modify CreditNote
	 * 
	 * @param sInput
	 *            String
	 */
	private void modifyCreditNote(String sInput) {
		try {
			if (!fiscalDocUtil.setNextCreditNoteNumber(sInput)) {
				theAppMgr.showErrorDlg(res.getString("CreditNote number already used, please try another"));
				selectModifyCreditNote();
				return;
			}
	...
	private void showPrintFiscalMenu() {
		theAppMgr.showMenu(MenuConst.NO_SALE_PRINT_FISCAL_DOCUMENT, "PRINT_FISCAL_NOSALE", theOpr);
	...
	private boolean postNoSaleTxn() {
		try {
			...
			if (theTxn.getCurrFiscalDocument() != null)
				return true;
			String txnNum = CMSTransactionNumberHelper.getNextTransactionNumber(theAppMgr, (CMSStore) theStore, (CMSRegister) theAppMgr.getGlobalObject("REGISTER"));
			theTxn.setId(txnNum);
			...
			if (theTxn != null && theTxn instanceof CMSCompositePOSTransaction) {
				((CMSCompositePOSTransaction) theTxn).doSetASISTxnData(asisTxnData);
	...
	public void appButtonEvent(CMSActionEvent anEvent) {
		...
		//10/31/07 Fix for 1864 :1.2.During this kind of transaction we need to forbid the action “Reservations”.
		if (sAction.equals("RESERVATIONS")) {
				if((theTxn.getTaxExemptId()!=null)&&(theTxn.getTaxExemptId().trim().length()>0)){
		...
		if (sAction.equals("PRINT_FISCAL_NOSALE")) {
			...
			//Fix for 1825: NoFiscalNoSale transaction saved in DB without Customer ID !!!
			if (theTxn != null && theTxn instanceof CMSCompositePOSTransaction) {
				if (theTxn.getCustomer() == null) {
					return;
				}
		...
		if (sAction.equals("OPTIONS_ALTERATIONS") || sAction.equals("ALTERATIONS")) {
			...
			if (lineItemHasAlteration(true)) {
				...
				if (alterationIDDlg.getSelectedAlterationID().equals("ARM_NEW_ALTERATION")) {
					enterAlterationID();
					theAppMgr.addStateObject("ALTERATION_SUB_GROUP_ID", ((CMSItem) pnlLineItem.getSelectedLineItem().getItem()).getProductNum());
					anEvent.consume();
					return;
				}
		...
		if (sAction.equals("SALE_TXN_RESERVATIONS")) {
			...
			if (pnlLineItem.getSelectedLineItem() != null && pnlLineItem.getSelectedLineItem().isMiscItem() && !LineItemPOSUtil.isNotOnFileItem(pnlLineItem.getSelectedLineItem().getItem().getId())) { // if
																																																		// Not-on-File
																																																		// Item)
		...
		//End: added by Himani for GC Balance Inquiry
			...
			goPrevious(anEvent); /*
									 * if (vecMenus.size() > 1) { appButtonEvent(new CMSActionEvent(anEvent.getSource(), 0, ( (String)
									 * vecMenus.elementAt(vecMenus.size() - 2)), 0)); vecMenus.removeElementAt(vecMenus.size() - 2);
									 * vecMenus.removeElementAt(vecMenus.size() - 1); } else { if (sAction_global != null) { showButtons();
									 * enterItem(); vecMenus = new Vector(); sAction_global = new String(); } else { theAppMgr.goBack(); } }
									 */
			...
			if (((CMSStore) theStore).usesRegionalTaxCalculations()) {
				...
				if (theTxn.getCustomer() != null) {
					...
					SwingUtilities.invokeLater(new Runnable() {
						 ...
						 */
						public void run() {
							theAppMgr.addStateObject("TAX_EXEMPT", "TAX_EXEMPT");
							theAppMgr.fireButtonEvent("ADD_CUSTOMER");
						}
					});
				}
		...
		// Return Rule
			...
			if(sCombineSaleandReturn!=null && sCombineSaleandReturn.equalsIgnoreCase("false")){
			if (theTxn.getSaleLineItemsArray().length > 0 && theTxn.getReturnLineItemsArray().length > 0) {
				theAppMgr.showErrorDlg(res.getString("NO SALE AND RETURN  ROWS  : is not possible to continue"));
				anEvent.consume();
				return;
			}
			//Ends
			}	
			if (theTxn.getSaleLineItemsArray().length > 0 && theTxn.getReturnLineItemsArray().length > 0 && theTxn.getCompositeNetAmount().doubleValue() < 0) {
				// Return and Sale items with refund due not allowed
				theAppMgr.showErrorDlg(res.getString("Sale and Return Items not allowed with refund due"));
				anEvent.consume();
				return;
			}
			ASISTxnData asisTxnData = (ASISTxnData) theAppMgr.getStateObject("ASIS_TXN_DATA");
			...
			if ((saleHasAlterationItems() && theTxn.getCustomer() == null) || saleHasRedeemableItems() || theTxn.isEmployeeSale) {
			...
			totalPressed = true;
			try {
				VATUtilities.applyVAT(theAppMgr, theTxn, (CMSStore) theStore, (CMSStore) theStore, theTxn.getProcessDate());
			...
			if (theTxn != null && theTxn instanceof CMSCompositePOSTransaction) {
				...
				if(applyTransactionLimits((CMSCompositePOSTransaction)theTxn)){
					theAppMgr.addStateObject("ARM_TRANSACTION_AMOUNT_OVER_LIMIT", "TRUE");					
				}else{
					theAppMgr.removeStateObject("ARM_TRANSACTION_AMOUNT_OVER_LIMIT");
				}
			}
			...
			if (priceConfig.getString("ENABLE_PRICE_OVERRIDE_BUTTON") != null) {
				enablePriceOverride = "true".equalsIgnoreCase(priceConfig.getString("ENABLE_PRICE_OVERRIDE_BUTTON"));
				if (!enablePriceOverride) {			
					theAppMgr.showErrorDlg(res.getString("Price override not permitted."));
					return;
				}
			}
			if (pnlLineItem.isRowSelected()) {
	...
	private void updateLabel() {
		...
		// if (amtRSVODeposit != null && bUpdateDepositAmount && isReservationCloseMode()) {
		...
		/*
		 * if (isNoSaleMode()) { sTotalAmount = new ArmCurrency(0.0d).formattedStringValue() + " / " +
		 * theTxn.getCompositeTotalAmountDue().formattedStringValue(); } else
		 */
		{
			// sTotalAmount = theTxn.getCompositeTotalAmountDue().formattedStringValue() + " / "
		...
		fldTotalUnits.setText(res.getString("Total Items") + " " + (theTxn.getCompositeTotalQuantityOfItems() - theTxn.getVoidLineItemsArray().length));
		if (theTxn.isPostAndPack()) {
			...
			try {
				...
				for (int i = 0; i < line.length; i++) {
					fldDeposit.setText(line[i].getNetAmount().formattedStringValue());
				}
				// System.out.println("***************fld " + theTxn.getDepositLineItems() + " { " +fldDeposit.getText() + " " +
				...
				ArmCurrency sTot = theTxn.getCompositeNetAmount().subtract(new ArmCurrency(fldDeposit.getText()));
					sTotalAmount = sTot.formattedStringValue() + " / " + new ArmCurrency(fldDeposit.getText()).formattedStringValue();
					fldSubTotal.setText(sTotalAmount);
				}
			} catch (Exception ex) {
				ex.printStackTrace();
		...
		// The fireTableDataChanged() removes the row selection somehow .. hence the code below
	...
	private void updateDiscountLabel() {
		...
		// on the applet
		/*
		 * Discount[] discounts = theTxn.getDiscountsArray(); if (discounts != null && discounts.length > 0) { Discount dis =
		 * discounts[discounts.length - 1]; StringBuffer buf = new StringBuffer(""); // buf.append(CMSDiscountMgr.getLabel(dis.getType()) + " - ");
		 * buf.append(dis.getType() + " - "); if (dis.isDiscountPercent()) { //double percent = dis.getPercent() * 100; //buf.append(percent + "%");
		 * NumberFormat percentFormat = NumberFormat.getPercentInstance(); percentFormat.setMinimumFractionDigits(2);
		 * buf.append(percentFormat.format(dis.getPercent())); } else { buf.append(dis.getAmount().formattedStringValue()); }
		 * labDiscount.setText(buf.toString()); } else { labDiscount.setText(""); }
		 */
		if (currPoleDisplayItem != null) {
			if (currPoleDisplayItem instanceof CMSReturnLineItem)
				LightPoleDisplay.getInstance().itemReturned(currPoleDisplayItem, theTxn.getCompositeTotalAmountDue().formattedStringValue());
			else
				LightPoleDisplay.getInstance().itemSold(currPoleDisplayItem, theTxn.getCompositeTotalAmountDue().formattedStringValue());
		}
	...
	private void deleteItem() {
		if (pnlLineItem.isRowSelected()) {
			...
			try {
				pnlLineItem.getTable().getSelectionModel().removeListSelectionListener(selectionResetListener);
				if ((isReservationCloseMode() || isNoOpenReservationCloseSaleMode()) && lineItmDeposit == pnlLineItem.getSelectedLineItem()) {
					miscDepositItem = null;
					amtRSVODeposit = null;
					bUpdateDepositAmount = false;
					lineItmDeposit = null;
	...
	/**
	 * Moved this prompt to a builder because this applet will never hear when the Eu presses <Enter> to remove the tax ID.
	 */
	private void buildTaxExemptId(String header) {
		theAppMgr.buildObject(this, header, "com.chelseasystems.cs.swing.builder.VATTaxIdBldr", null);
	}
	...
	private void showButtons() {
		if (isNoSaleMode()) {
			...
			if (theAppMgr.getStateObject("RETURN_MODE") != null && theAppMgr.getStateObject("RETURN_TXN_POS") == null) {
	...
	/**
	 * initHeader initializes the OperatorName, ConsultantName and CustomerName and Phone
	 */
	...
	private boolean isNoSaleMode() {
		return (theAppMgr.getStateObject("TXN_MODE") != null && ((Integer) theAppMgr.getStateObject("TXN_MODE")).intValue() == NO_SALE_MODE);
	}
	...
	private boolean isNoReturnMode() {
		return (theAppMgr.getStateObject("TXN_MODE") != null && ((Integer) theAppMgr.getStateObject("TXN_MODE")).intValue() == NO_RETURN_MODE);
	}
	...
	private boolean isNoOpenReservationCloseSaleMode() {
		return (theAppMgr.getStateObject("TXN_MODE") != null && ((Integer) theAppMgr.getStateObject("TXN_MODE")).intValue() == NO_OPEN_RESERVATIONS_CLOSE_SALE);
	}
	...
	private boolean isNoOpenReservationCloseReturnMode() {
		return (theAppMgr.getStateObject("TXN_MODE") != null && ((Integer) theAppMgr.getStateObject("TXN_MODE")).intValue() == NO_OPEN_RESERVATIONS_CLOSE_RETURN);
	}
	...
	private void calculateDepositAmount(boolean bUpdateLabels) {
		...
		ArmCurrency amtDeposit = null;
			...
			// being returned.
			if (theTxn.getSaleLineItemsArray().length < 1 && theTxn.getLineItemsArray().length == theTxn.getReturnLineItemsArray().length) {
				amtDeposit = amtRSVODeposit.absoluteValue();
			}
	...
	private MiscItemTemplate solicitDepositItemTemplateChoice(boolean bOpenDeposit) {
		...
		MiscItemTemplate[] miscDepositTemplates = vecMiscItems.toArray(new MiscItemTemplate[vecMiscItems.size()]);
	 ...
	 */
	//Deepika 
	//Modification done in solicitMiscItemTemplateChoice() to display the service menus based on the roles defined in item.cfg.
	...
	private void jbInit() throws Exception {
		...
		pnlSouth.add(lblFiscalReceiptNumber, new GridBagConstraints(0, 1, 1, 1, 1.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 5, 5, 5), 10, 0));
		pnlSouth.add(this.lblEMployeeSale, new GridBagConstraints(1, 1, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 5, 5, 5), 0, 0));
		...
		/*
		 * ---For Testing layout purposes //--- pnlSouth.setBorder( BorderFactory.createLineBorder(Color.red)); labSubTotal.setBorder(
		 * BorderFactory.createLineBorder(Color.green)); lblLoyality.setBorder( BorderFactory.createLineBorder(Color.blue)); labDeposit.setBorder(
		 * BorderFactory.createLineBorder(Color.black)); fldSubTotal.setBorder( BorderFactory.createLineBorder(Color.lightGray));
	...
	private void goPrevious(CMSActionEvent anEvent) {
		...
		if (vecMenus.size() > 1) {
			appButtonEvent(new CMSActionEvent(anEvent.getSource(), 0, vecMenus.elementAt(vecMenus.size() - 2), 0));
	 ...
	 *            bBuildAlterationTickets = true --> If dailog box with alterationids needs to be shown bBuildAlterationTickets = false --> Dialog box
	 *            not needed.
	 * @return boolean
	 */
	...
	/**
	 * Customer is required for tenderign If there is any alteration item in sale.
	 * 
	 * @return boolean
	 */
	...
	/**
	 * Load all the Altertaion ITEM_GROUP_NAMES from Alteration.xml into popup for customer owned merchandise item and solicit alteration on it.
	 */
	private void solicitCustomerOwnedAlteration() {
		Hashtable<String, String> htGroups = (new AlterationLookUpUtil().getAlterationGrpNamesWithDefaultSubGroupId());
		// No Groups available
		if (htGroups == null) {
		...
		try {
			...
			if (dis.getType().equals("BY_PRICE_DISCOUNT")) {
				if (lineItem instanceof CMSSaleLineItem) {
					...
					if (((CMSNoSaleLineItem) lineItem).isPriceDiscountAdded && ((CMSNoSaleLineItem) lineItem).getPriceDiscount() != null) {
						priceLineDiscount = ((CMSNoSaleLineItem) lineItem).getPriceDiscount();
						((CMSNoSaleLineItem) lineItem).removeDiscount(priceLineDiscount);
						theTxn.removeDiscount(priceLineDiscount);
					...
					if (((CMSNoReturnLineItem) lineItem).isPriceDiscountAdded && ((CMSNoReturnLineItem) lineItem).getPriceDiscount() != null) {
						priceLineDiscount = ((CMSNoReturnLineItem) lineItem).getPriceDiscount();
						((CMSNoReturnLineItem) lineItem).removeDiscount(priceLineDiscount);
						theTxn.removeDiscount(priceLineDiscount);
						((CMSNoReturnLineItem) lineItem).removeAddPriceDiscount();
					}
	...
	private void updateMerchandiseCountLabel() {
		labMerchandiseCount.setText(res.getString("Merchandise Count: ") + (this.theTxn.getTotalMerchandiseCount() - theTxn.getVoidLineItemsArray().length));
	}
	...
	private boolean checkAlterationValid() {
		try {
			if (theAppMgr.getStateObject("POS_LINE_ITEM") != null) {
				...
				String productNum = ((CMSItem) posLineItem.getItem()).getProductNum();
				if (!(posLineItem.isMiscItem() && LineItemPOSUtil.isNotOnFileItem(posLineItem.getItem().getId())) // make sure item is not
																													// not-on-file
		...
		if (miscItemTemplate != null) {
			if (miscItemTemplate.getKey().equals("STORE_VALUE_CARD")) {
				if (iAppletMode == RETURN_MODE){
					...
					theAppMgr.showErrorDlg(res.getString("This function is not available in Offline Mode"));
				}
				else {
					try {
	...
	protected boolean applyTransactionLimits(CMSCompositePOSTransaction theTxn){
		...
		{
			ConfigMgr configMgr = new ConfigMgr("payment.cfg");
			String paymentLimit = configMgr.getString("TRANSACTION_PAYMENT_AMOUNT_LIMIT");
			
			if(paymentLimit !=null){
				//theTxn.getCompositeRetailAmount().
				applyLimits = theTxn.getCompositeNetAmount().greaterThanOrEqualTo(new ArmCurrency(paymentLimit));
			}
