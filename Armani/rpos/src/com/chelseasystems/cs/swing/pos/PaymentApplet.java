...
import com.chelseasystems.cs.paidout.CMSPaidOutTransactionAppModel;
...
import com.chelseasystems.cs.swing.builder.RedeemableBldrUtil;
			...
			try {
				if(rownum > -1 && OrgSaleTxn!=null){
					...
					for(int i = 0 ;i<model.getRowCount();i++){
						System.out.println("removing i store value card1111"+i);
						if(model.getRowInPage(i) instanceof StoreValueCard && isGiftCardReturn ){
							System.out.println("removing i store value card"+i);
							model.removeRow(i);
							i= -1;
						}
									...
									if(trackData!=null && trackData!="")
									{
										...
										if(!activationResponse)
										{
											...
											retryConnect = theAppMgr.showOptionDlg("Fipay Error"
													 			    	            ,  CMSCreditAuthHelper.getResponseErrorMsg()+". Gift Card relaod failed. Please try again or Void this transaction.", "Retry", "Void");
											 CMSCreditAuthHelper.setResponseErrorMsg("");
											if(retryConnect)	
											{
												i = i-1;
            ...
            //Added changes for new Return flow. 
		      ...
		      //Mayuri Edhara fix for Change due name
				...
				if(token != null){
					...
					theAppMgr.showErrorDlg("The original credit card transaction was not processed on the pinpad.Please process this refund using a “No Receipt Return”.");
					return; 
					}
				}
				else{
          ...
          if(!response)
			{
				...
				retryConnect = theAppMgr.showOptionDlg("Fipay Error"
						 			    	            , CMSCreditAuthHelper.getResponseErrorMsg()+". Unable to void Redeemable Payment. Please select Retry to try againg or Cancel to POST VOID the transaction later.", "Retry", "Cancel");
				CMSCreditAuthHelper.setResponseErrorMsg("");
				if(retryConnect)	
				{
