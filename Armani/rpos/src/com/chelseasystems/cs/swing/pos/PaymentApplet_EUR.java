...
public class PaymentApplet_EUR extends CMSApplet implements Setefi
{
	...
	//Vishal Yevale : If appears the Budget Message , the VIP message is not required. 29th May 2017
	...
	private void jbInit() throws Exception {
		...
		for (int i = 0; i < model.getColumnCount(); i++) {
			modelColumn.getColumn(i).setCellRenderer(paymentRenderer); // where renderer is an instance of your renderer class
		}
					...
					if (object instanceof CMSCompositePOSTransaction) {
					    CMSCompositePOSTransaction theTxnPOS = (CMSCompositePOSTransaction) object;
						 ...
						 if (customerType != null && customerType.equals(CustomerSearchString.CUSTOMER_TYPE_EMPLOYEE_CODE)) {
							 try{
								 CMSCustomer customer = (CMSCustomer) theTxnPOS.getCustomer();
									String countryCode = (theTxnPOS.getStore()).getPreferredISOCountry();
	...
	public void start() {
				...
				if (theAppMgr.getStateObject("TXN_CUSTOMER") != null) {
						 ...
						 if (customerType != null && customerType.equals(CustomerSearchString.CUSTOMER_TYPE_EMPLOYEE_CODE)) {
							 try{
								 CMSCustomer customer = (CMSCustomer) theTxnPOS.getCustomer();
								    ...
								    if(customerAlertRules!=null)
									 ruleDiscountPct = String.valueOf(Double.parseDouble(customerAlertRules[0].getDsc_level()));
									String validDiscounts = "";
									String records = "";
									boolean flag = false;
								    if(customerAlertRules!=null && customerAlertRules.length > 1){
										for (int countOne = 0; countOne < customerAlertRules.length; countOne++) {
											if(!records.contains(String.valueOf(countOne))){
									    		for( int countTwo = countOne+1; countTwo < customerAlertRules.length; countTwo++){
									    			if(!records.contains(String.valueOf(countTwo))){
										    			if((customerAlertRules[countOne].getPriority().intValue() == customerAlertRules[countTwo].getPriority().intValue()) && (customerAlertRules[countOne].getRecordType().toString().equalsIgnoreCase(customerAlertRules[countTwo].getRecordType().toString())) && (customerAlertRules[countOne].getValue().toString().equalsIgnoreCase(customerAlertRules[countTwo].getValue().toString()))){
										    				if(!validDiscounts.contains(String.valueOf(Double.parseDouble(customerAlertRules[countTwo].getDsc_level()))))
											    				validDiscounts += String.valueOf( Double.parseDouble(customerAlertRules[countTwo].getDsc_level()))+",";
											    			records +=  String.valueOf(countTwo)+",";
											    			flag =true;
										    			}
									    			}
									    		}
									    		if(flag){
									    			if(!validDiscounts.contains(String.valueOf(Double.parseDouble(customerAlertRules[countOne].getDsc_level()))))
									    				validDiscounts += String.valueOf(Double.parseDouble(customerAlertRules[countOne].getDsc_level()));
									    		}
									    		flag = false;
											}
										}
							    	...
							    	if(validDiscounts != null || validDiscounts != "")
								     ...
								     for(int i=0;i<lineItm.length;i++){
								    	...
								    	double discountpct = lineItm[i].getDiscount().getPercent();
								    	//if(((discountpct*100)==ruleDiscountPct) || (ruleDiscountPct == 0.0) ){
								      	if((ruleDiscountPct.contains(String.valueOf(discountpct*100))) || (ruleDiscountPct == null) || (ruleDiscountPct == "")){
								    	   	counter = counter+1;
					...
					/*	  try {
							...
							try {
					//PCR: Dummy or default customer not allowed for Reservation,
					//Deposit and Credid transactions
					theTxnPOS.setCustomer(txnCust);
					VATUtilities.applyVAT(theAppMgr, theTxnPOS, (CMSStore) theStore, (CMSStore) theStore, theTxnPOS.getProcessDate());
					//        TaxUtilities.applyTax(theAppMgr, theTxnPOS,
					//                              (CMSStore) theTxnPOS.getStore(),
					//                              (CMSStore) theTxnPOS.getStore(),
					//                                  theTxnPOS.getProcessDate());
					//sonali:priceOveride
					CMSCompositePOSTransaction	trans = (CMSCompositePOSTransaction) theAppMgr.getStateObject("TXN_POS");

				    POSLineItem[] lineItem=trans.getSaleLineItemsArray();
					for(int i=0;i<lineItem.length;i++)
					{if(lineItem[i].getOverrideAmount()!=null)
						{
						lineItem[i].setManualUnitPrice(lineItem[i].getOverrideAmount());
		...
		if (theAppMgr.getStateObject("ARM_PRINT_FISCAL_MODE") != null) {
			...
			System.out.println("======================Back From Print Fiscal Applet===================");
			bBackFromPrintFiscalApplet = true;
		} else {
			System.out.println("========================Not Back From Print Fiscal Applet===============");
		}
		...
		if (theTxn.getCustomer() != null) {
				//Vishal Yevale : If appears the Budget Message , the VIP message is not required. 29th May 2017
				if (((CMSCustomer) theTxn.getCustomer()).getVIPDiscount() > 0 && proceedVIPdiscountMsg) {
				//end Vishal Yevale 29 May 2017
				...
				theAppMgr.showErrorDlg(res.getString("Customer has VIP discount: ") + nf.format(((CMSCustomer) theTxn.getCustomer()).getVIPDiscount() / 100));
			}
			//      if (giveCustomerVIPDiscount != null && giveCustomerVIPDiscount.equals("TRUE")
		...
		if (bBackFromPrintFiscalApplet) {
			System.out.println("===================Print Fiscal Mode==================");
			theAppMgr.showMenu(MenuConst.PRINT_FISCAL_DOCUMENT, "PRINT_FISCAL", theOpr);
		...
		if (custNeededPaymentObj != null && custNeededPaymentObj instanceof Payment && theTxn.getCustomer() != null && theTxn.getCustomer().getId() != null
		...
		{
		    if (!theAppMgr.showOptionDlg("Errore","Errore Stampante Fiscale.Riaccendere la stampante e rifare lo scontrino.")) {
				return;
			}
		}else{
	...
	/**
	 * Creates Transaction Number
	 * For Europe: Gets the fiscal number
	 */
	private void createTxnNumber() {
		try {
			...
			if (fiscalReceiptNumber == null || fiscalReceiptNumber.length() == 0) {

				//     fiscalReceiptNumber = fiscalInterface.getNextFiscalReceiptNo();

				//    	  String fiscalRcptNumber = (String)theAppMgr.getStateObject("FISCAL_RCPT_NUMBER");
	...
	private void initHeaders() {
		try {
			...
			// MP: In case of PaidOut, no customer associated just the Employee.
				...
				if((CMSCustomer) theTxn.getCustomer()== null && !theAppMgr.isOnLine()){
				CMSCustomer cust = 	((CMSCustomer)theAppMgr.getStateObject("OFFLINE_CUST"));
				if(cust!=null){
				pnlHeader.setProperties(cust, (CMSEmployee) theOpr, (CMSEmployee) theTxn.getConsultant(), (CMSStore) theStore, theTxn.getTaxExemptId(),
	...
	private void initButtons(int type) {
		...
		for (int x = 0; x < pay.length; x++) {
				...
				if (pay[x].equalsIgnoreCase("CREDIT_CARD")) {
					btns[x] = new JButton(HTML.formatLabeltoHTML(res.getString("Credit Card/Debit Card"), theAppMgr.getTheme().getButtonFont(), theAppMgr.getTheme().getButtonTextColor()));
				} else {
				...
				if (pay[x].equalsIgnoreCase("STORE_VALUE_CARD")) {
					btns[btns.length-1] = new JButton(HTML.formatLabeltoHTML(res.getString("Gift Card"), theAppMgr.getTheme().getButtonFont(), theAppMgr.getTheme().getButtonTextColor()));
		...
		//added by shushma for disable payment option, if no payment is enabled and pay throws NullPointerException
		catch(NullPointerException npe){
			theAppMgr.showErrorDlg("All payment options are disabled. Please enable at least one payment option in payment.cfg and restart Client. Shutting Down....");
			System.exit(1);
		}
		catch (Exception ex) {
	...
	public void editAreaEvent(String theCommand, ArmCurrency currency) {
		if (theCommand.equals("CREDIT_CARD_AMOUNT")) {
			if (TransactionUtil.validateChangeAmount(theAppMgr, "CREDIT_CARD", "CREDIT_CARD", currency)) {
				this.creditCardAmountForSetefi = currency;
				double p = 0.0d;
				p = currency.doubleValue();
				try {
					SetefiManagement sm = new SetefiManagement(this, new Double(p));
					theAppMgr.unRegisterSingleEditArea();
					theAppMgr.setSingleEditArea(res.getString("Credit Auth in progress..."));
					isCreditAuthComplete = false;
//					theAppMgr.getParentFrame().toBack();
					sm.createLink(sm.createJposEntry(), true);
					//fireSetefiResult(sm);
					//theAppMgr.showErrorDlg("Contacting Setefi Gateway !!!");
					theAppMgr.unRegisterSingleEditArea();
					return;
				} catch (Exception e) {
					...
					System.out.println("the builder is: " + Builder);
					theAppMgr.buildObject("PAYMENT", Builder, currency);
					return;
				}
			} else {
				try {
				ArmCurrency amt = theTxn.getCompositeTotalAmountDue().subtract(theTxn.getTotalPaymentAmount());
					theAppMgr.setSingleEditArea(res.getString("Enter amount....."), "CREDIT_CARD_AMOUNT", amt.absoluteValue(), theAppMgr.CURRENCY_MASK);
				} catch (Exception e) {
					theAppMgr.setSingleEditArea(res.getString("Enter amount....."), "CREDIT_CARD_AMOUNT", theAppMgr.CURRENCY_MASK);
					e.printStackTrace();
				}
			}
		}
	...
	public void objectEvent(String Command, Object obj) {
		if (Command.equals("PAYMENT")) {
			...
			/*	//Vishal Yevale : Added for saving CREDIT_CARD rather than it's Merchant name : 6 March 2017
					if(obj instanceof CMSCreditCard){
					...
					if (pay.getPaymentCode() == null || pay.getPaymentCode() == "" || pay.getPaymentCode().trim().length() == 0) {
						CMSPaymentCode payCode = getPaymentCode(pay.getTransactionPaymentName());
						if (payCode != null) {
					...
					if (!pay.getAmount().equals(new ArmCurrency(0.0))) {
						...
						try {
							...
							model.fireTableDataChanged();
							updateLabels();
							//ks: Europe - "Choose change type" replaced with "Select tender."
		...
		if (Command.equals("ISSUERWDCARD")) {
			...
			long points = (new Long(loyaltyRewardRatio)).longValue() * (new Long(loyaltyAmount)).longValue();
	...
	private void applyVat(boolean forCredit) throws Exception {
		if (isVatEnabled) {
			if (theTxn.getPaymentTransaction() instanceof CMSCompositePOSTransaction) {
				...
				VATUtilities.applyVAT(theAppMgr, (CMSCompositePOSTransaction) theTxn.getPaymentTransaction(), (CMSStore) theStore, (CMSStore) theStore, theTxn.getPaymentTransaction().getProcessDate());
	...
	private void selectPayment() {
		try {
		ArmCurrency amt = theTxn.getCompositeTotalAmountDue().subtract(theTxn.getTotalPaymentAmount());
			//MP: Changed the code as the comparison was not working fine in case TaxExempt was added.
		...
		ArmCurrency absVal = amt.absoluteValue();
			...
			if (amtStr.equals(valStr) || amt.equals(val) || absVal.stringValue().equals(valStr)) {
				if (shouldTxnPostWhenBalanceIsZero) {
					Thread post = new Thread(new Runnable() {
						...
						public void run() {
							...
							PaymentApplet_EUR.this.appButtonEvent(new CMSActionEvent(this, 0, "POST_TXN", 0));
					...
					theAppMgr.showMenu(MenuConst.POST_TXN_EU, "PRINT_FISCAL_RECEIPT", theOpr);
					PaymentApplet_EUR.this.appButtonEvent(new CMSActionEvent(this, 0, "PRINT_FISCAL_RECEIPT", 0));
					//          selectOption();
					//PaymentApplet_EUR.this.appButtonEvent(new CMSActionEvent(this, 0, "PRINT_FISCAL_RECEIPTT, 0));
			...
			if (amt.lessThan(new ArmCurrency(0.0))) {
				...
				if (theTxn.getCompositeTotalAmountDue().doubleValue() < 0) {
					...
					ArmCurrency amtDue = theTxn.getCompositeTotalAmountDue().subtract(theTxn.getTotalPaymentAmount());
						if (amtDue.equals(new ArmCurrency(0.0))) {
							...
							theAppMgr.showMenu(MenuConst.POST_TXN_EU, theOpr, theAppMgr.PREV_BUTTON);
							selectOption();
							updateLabels();
							changeDue = true;
							return;
				...
				if (bool) {
					if (theAppMgr.getStateObject("PAYMENT") != null) {
						SwingUtilities.invokeLater(new Runnable() {
							 ...
							 */
							public void run() {
								PaymentApplet_EUR.this.appButtonEvent(new CMSActionEvent(this, 0, (String) theAppMgr.getStateObject("PAYMENT"), 0));
				...
				// MP: In Eur the INITIAL_PAYMENT is null, so when we return from
				// customer-lookup, we need to check the 'PAYMENT' stateObj.
				// If it doesn't equal to null fire the event (In our case it is to
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						SwingUtilities.invokeLater(new Runnable() {
							 ...
							 */
							public void run() {
								if (bool && !flag && isButtonInList(sAction_global) && theAppMgr.getStateObject("PAYMENT") != null) {
									PaymentApplet_EUR.this.appButtonEvent(new CMSActionEvent(this, 0, (String) theAppMgr.getStateObject("PAYMENT"), 0));
									theAppMgr.removeStateObject("PAYMENT");
								}
							}
					...
					try {
						...
						SwingUtilities.invokeLater(new Runnable() {
							...
							public void run() {
								...
								if (!bool) {
									PaymentApplet_EUR.this.appButtonEvent(new CMSActionEvent(this, 0, INITIAL_PAYMENT, 0));
								} else {
									if (flag) {
										PaymentApplet_EUR.this.appButtonEvent(new CMSActionEvent(this, 0, INITIAL_PAYMENT, 0));
									} else if (isButtonInList(sAction_global)) {
										System.out.println("******** In else statement when bool = T & flag = F ");
										// for skip condition in CustListApplet to return to PaymentApplet_EUR
										PaymentApplet_EUR.this.appButtonEvent(new CMSActionEvent(this, 0, (String) theAppMgr.getStateObject("PAYMENT"), 0));
									} else {
										PaymentApplet_EUR.this.appButtonEvent(new CMSActionEvent(this, 0, INITIAL_PAYMENT, 0));
									}
	...
	private boolean postTransaction() throws BusinessRuleException {
		try {
			if (!isFiscalReceiptPrinted)
				theAppMgr.showErrorDlg(res.getString("Please prepare manual receipt for customer"));
			...
			try {
				if (shouldDrawPop()) {
					System.out.println("PaymentApplet_EUR.postTransaction()->Pop Drawer...");
...
//				String activateDigitalSignature = cfgMgr1.getString("ACTIVATE_DIGITAL_SIGNATURE");
...
//						// To get any previous digital signature if any from TR_RTL table in order to create the message
//						String previousDigitalSignature = (String)CMSTransactionPOSHelper.selectDigitalSignature(theAppMgr, txnId);
//						String digitalSignature = (DigitalSignatureUtil.createMessageAndDigitalSignature(txn, previousDigitalSignature));
...
//				 */
		...
		//Poonam:Added for saving CREDIT_CARD to DB instead of it's Merchant name. 
						...
						if(payment[i] instanceof CMSCreditCard){
			            	...
			            	if(model!=null){
						        ...
						        for(int count=0;count<rows;count++){
								    ...
								    Payment pay= (Payment)vTemp.elementAt(count);
								    // start Bug #:-28022-saptarshi - putting extra null check on payment code-								  
								    if(pay!=null && pays!=null){
								    if((pay.getPaymentCode()!=null) && (pays.getPaymentCode()!=null) && (pay.getPaymentCode().equalsIgnoreCase(pays.getPaymentCode()))){
								    //end Bug #:-28022-saptarshi - putting extra null check on payment code-
								    	model.removeRow(count);
								    	model.addPayment(pays);
								        model.fireTableDataChanged(); 
								    }
								    }////end bug no:-28022-saptarshi - putting extra null check on payment code-
						        }
				            }
						}
					}
	...
	//Poonam:Added to show actual card type on receipt
			...
			for (int i=0;i<pay.length;i++) {
				...
				if(type != null && pay[i].getGUIPaymentName()!=null && (pay[i].getGUIPaymentName().contains("CREDIT"))){
			...
			if(theAppMgr.getStateObject("ARM_TRANSACTION_AMOUNT_OVER_LIMIT")!=null && theAppMgr.getStateObject("ARM_TRANSACTION_AMOUNT_OVER_LIMIT").equals("TRUE")){
				theAppMgr.showErrorDlg(res.getString("Please remember to print Fiscal document for this transaction"));
		...
		if (sAction.equals("PREV")) {
			...
			selectModifyCreditNote();
				...
				try {
					    ...
					    theAppMgr.showErrorDlg(res.getString("Mandatory add a Fiscal Document on this transaction"));
						...
						theAppMgr.addStateObject("NEW_TXN_AFTER_TENDER", "NEW_TXN_AFTER_TENDER");
	...
	public void appButtonEvent(CMSActionEvent anEvent) {
		//sonali:Changes for persisting discount code and discount percent ;setting Discounts in reduction array
		CMSCompositePOSTransaction txnObj = (CMSCompositePOSTransaction) theAppMgr.getStateObject("TXN_POS");
	     ...
	     /*   for(int k=0;k<returnItems1.length;k++)
	        {
	        	...
	        	SaleLineItem saleItem=((CMSReturnLineItem)returnItems1[k]).getSaleLineItem();
	        	if(saleItem!=null)
	        	{
	        		Discount[] discount=saleItem.getDiscountsArray();
	        	  if(discount!=null && discount.length>0)
	        	  	{
	        		  Reduction[] baseReductions=((POSLineItemDetail)returnItems1[k].getLineItemDetailsArray()[0]).getReductionsArray();
	        		  ((POSLineItemDetail)returnItems1[k].getLineItemDetailsArray()[0]).doRemoveAllReductions();

	        		  for(int l=0;l<discount.length;l++)
	        		  {
	        			  CMSReduction reduction=new CMSReduction(baseReductions[l].getAmount(),baseReductions[l].getReason());
	        			  reduction.doSetDiscount(discount[l])	;
	        			  ((POSLineItemDetail)returnItems1[k].getLineItemDetailsArray()[0]).doAddReduction(reduction);
	        	...
	        	*/
			 // Vishal : done the changes for persisting same discount rows in db for return transaction as Sale transaction 08-July-2016
			 for(int k=0;k<returnItems1.length;k++)
		        {
		        	...
		        	if(saleItem!=null)
		        	{
		        	  ...
		        	  if(discount!=null && discount.length>0)
		        		  ...
		        		  ((POSLineItemDetail)returnItems1[k].getLineItemDetailsArray()[0]).doRemoveAllReductions();
		        		  int temp=0;
		        		  int arr=(discount.length>=baseReductions.length)?discount.length:baseReductions.length;
		        		  for(int l=0,j=0;l<arr;l++,j++)
		        		  {
		        			  if(temp==0 && baseReductions.length>discount.length )
		        			  {
			        			  CMSReduction reduction=new CMSReduction(baseReductions[l].getAmount(),baseReductions[l].getReason());
			        			  ((POSLineItemDetail)returnItems1[k].getLineItemDetailsArray()[0]).doAddReduction(reduction);
			        			  temp++;
		        			  ...
		        			  else
		        			  {
								  ...
								  /* Bug no 28132:-start-checking for automatic discount, 
		        				   * automatic discount should not set in reduction  */
		        				  if(baseReductions==null || baseReductions.length<=0 || l>=baseReductions.length){ 
		        					  continue;
		        				  }	
		        				  /* Bug no 28132:-end-checking for automatic discount, 
		        				   * automatic  discount should not set in reduction  */
		...
		try {
			if (sAction.equals("PRINT_FISCAL")) {
				CMSCompositePOSTransaction theTxnPOS = (CMSCompositePOSTransaction) theAppMgr.getStateObject("TXN_POS");
				if (theTxnPOS != null && theTxnPOS.isTaxExempt()) {
					theAppMgr.showErrorDlg(res.getString("VAT Invoice is Compulsory"));
			...
			if (sAction.equals("NEW_TRANSACTION")) {
				try {
					...
					theAppMgr.addStateObject("OPERATOR", employee);
					//___Tim: check this state object from IdentifyConsultantApplet 
					//so that employee cannot return to payment applet after posting Txn
					theAppMgr.addStateObject("NEW_TXN_AFTER_TENDER", "NEW_TXN_AFTER_TENDER");
					} catch (Exception ex) {
					ex.printStackTrace();
					anEvent.consume();
			...
			if (sAction.equals("DEL_PAY")) {
				...
				if (txn instanceof CMSCompositePOSTransaction) {
					...
					if (OldObj != null) {
						...
						if (OldObj.getCurrBalance() + theTxnPOS.getLoyaltyPoints() > 1000) {
				...
				//___Tim: To prevent user from hitting 'ESC' during or after posting Txn
			...
			ArmCurrency amt = theTxn.getCompositeTotalAmountDue().subtract(theTxn.getTotalPaymentAmount());
				...
				try {
					createTxnNumber(); //creates a txn number if not already created.
					System.out.println("****----This is to test theTxn Object. Store id is: "+theTxn.getPaymentTransaction().getStore()+" ----****");
					System.out.println("****----This is to test theTxn Object"+theTxn.getPaymentTransaction().getId()+" ----****");
					  /**
					 * Added by Satin to create digital signature.
					 * Code change for digital signature starts here.
					 */
					ConfigMgr cfgMgr1 = new ConfigMgr("payment.cfg");
					String activateDigitalSignature = cfgMgr1.getString("ACTIVATE_DIGITAL_SIGNATURE");
					String austrianEnv = cfgMgr1.getString("AUSTRIAN_ENVOIREMENT");
					...
					if ((activateDigitalSignature.length())!=0)
					   {
						if ( activateDigitalSignature.toUpperCase().compareTo("FALSE")==0 )
						{
							//theAppMgr.showErrorDlg(res.getString("Specify the parameter ACTIVATE_DIGITAL_SIGNATURE in payment.cfg"));
						}
						else 
							...
							if (austrianEnv.toUpperCase().compareTo("FALSE")==0)
							{
										 ...
										 transtype = txn.getTransactionType(); 
							     		 ...
							     		 if (txn instanceof CMSMiscCollectionCredit) {
											    txnId = ((CMSMiscCollectionCredit)txn).getId();
										   }	
										   ...
										   if (txn instanceof CMSCompositePOSTransaction) {
											    txnId = ((CMSCompositePOSTransaction)txn).getId();  
										   }
										   
										  
										   if (txn instanceof CollectionTransaction) {
											   txnId = ((CollectionTransaction)txn).getId();  
										   }
										   
										  
										   String recnum = fiscalInterface.getNextFiscalReceiptNo();
										   
										   if  ( (transtype.equals("SALE"))||(transtype.equals("SALE/RETN")) )  {
												
												txnCompositeID ="FT "+Integer.toString(year)+shopId+cash_register+"/"+recnum;
												
											}
											if  (transtype.equals("RETN"))  {
												
												txnCompositeID ="NC "+Integer.toString(year)+shopId+cash_register+"/"+fiscalReceiptNumber;
											...
											// Riccardino modifica : messo nel nuovo metodo createNewMessageAndDigitalSignature anche la transazione composta (txnCompositeID)
											...
											if ((activateDigitalSignature.trim().equalsIgnoreCase("true"))&&(austrianEnv.trim().equalsIgnoreCase("false")))
											{
												
												// To get any previous digital signature if any from TR_RTL table in order to create the message
												//String previousDigitalSignature = (String)CMSTransactionPOSHelper.selectDigitalSignature(theAppMgr, txnId);
													if(txn.getTransactionType().equalsIgnoreCase("SALE")){
														previousSaleDigitalSignature = (String)theAppMgr.getGlobalObject("SALE_DIGITAL_SIGNATURE");
														
														if(previousSaleDigitalSignature.equals("0")){
															digitalSaleSignature = (DigitalSignatureUtil.createNewMessageAndDigitalSignature(txn, null,txnCompositeID));
															((CMSCompositePOSTransaction)txn).setDigitalSignature(digitalSaleSignature);
															theAppMgr.addGlobalObject("SALE_DIGITAL_SIGNATURE", digitalSaleSignature,true);
															System.out.println("digitalSignature IN SALE:"+digitalSignature);
														}else
															digitalSaleSignature = (DigitalSignatureUtil.createNewMessageAndDigitalSignature(txn, previousSaleDigitalSignature,txnCompositeID));
															((CMSCompositePOSTransaction)txn).setDigitalSignature(digitalSaleSignature);
															theAppMgr.addGlobalObject("SALE_DIGITAL_SIGNATURE", digitalSaleSignature,true);
															System.out.println("previousDigitalSignature IN SALE not for the first time:"+previousSaleDigitalSignature);
												}
													if(txn.getTransactionType().equalsIgnoreCase("RETN")){
														previousReturnDigitalSignature = (String)theAppMgr.getGlobalObject("RETURN_DIGITAL_SIGNATURE");
														
														if(previousReturnDigitalSignature.equals("0")){
															digitalReturnSignature = (DigitalSignatureUtil.createNewMessageAndDigitalSignature(txn, null,txnCompositeID));
														((CMSCompositePOSTransaction)txn).setDigitalSignature(digitalReturnSignature);
														theAppMgr.addGlobalObject("RETURN_DIGITAL_SIGNATURE", digitalReturnSignature,true);
														System.out.println("digitalSignature IN RETURN:"+digitalReturnSignature);
														}else
															digitalReturnSignature = (DigitalSignatureUtil.createNewMessageAndDigitalSignature(txn, previousReturnDigitalSignature,txnCompositeID));
															((CMSCompositePOSTransaction)txn).setDigitalSignature(digitalReturnSignature);
															theAppMgr.addGlobalObject("RETURN_DIGITAL_SIGNATURE", digitalReturnSignature,true);
															System.out.println("previousDigitalSignature IN RETURN not for the first time:"+previousReturnDigitalSignature);
													}	
											}
											
							
							//String digitalSignature = (DigitalSignatureUtil.createNewMessageAndDigitalSignature(txn, previousDigitalSignature,txnCompositeID));
							
						//}
					  } else // AUSTRIA CERTIFICATION 
					  		{
						     String txnId ="0";
							 String transtype ;
							 String txnCompositeID ="0";
							...
							//PRENDO AESKEY  
							  Aeskey= (String)theAppMgr.getGlobalObject("AES_KEY");
							  System.out.println("The ASK_KEY Stored:"+Aeskey);
				     		 ...
				     		 if (txn instanceof CMSMiscCollectionCredit) {
							   ...
							   if (txn instanceof CollectionTransaction) {
								...
								if ((activateDigitalSignature.trim().equalsIgnoreCase("true"))&&(austrianEnv.trim().equalsIgnoreCase("true")))
								{
												
												//"AES_KEY" "CARD_CERTIFICATE" "TURN_OVER_AMT"			        	
												
									        //Calcolo il total takings 
												previousTurnOver = (String)theAppMgr.getGlobalObject("TURN_OVER_AMT");
												//actualTurnOver =DigitalSignatureUtil.sumTurnover(txn,previousTurnOver);
												theAppMgr.addGlobalObject("TURN_OVER_AMT", actualTurnOver,true);
												System.out.println("Turnover CASH REGISTER Stored:"+actualTurnOver);
											 // Ora Passo tutto nel Encripted 
												
											// PRENDO IL TOTALE E LO METTO IN HASH
											
												//encripted = DigitalSignatureUtil.getTurnoverEncripted(Aeskey, txnId, shopId, cash_register,actualTurnOver);
									     	
											// PRENDO LA STRINGA HASH PRECEDENTE 	 
												previousSaleDigitalSignature = (String)theAppMgr.getGlobalObject("SALE_DIGITAL_SIGNATURE");
											// CALCOLA LA DIGITAL SIGNATURE E METTO IN ARRAY :  0 = DIGITAL SIGNATURE / 1 = FALLEN 
												//digitalSaleSignature = (DigitalSignatureUtil.createAustriaDigitalSignature(txn,shopId,cash_register,previousSaleDigitalSignature,txnId,encripted,myCertificate,isReturn));
												
											// AGGIUNGO NELLA TRANSAZIONE POSTRANSACTION LA DIGITALSIGNATURE 	 
												// ((CMSCompositePOSTransaction)txn).setDigitalSignature(digitalSaleSignature[0]);
												 //((CMSCompositePOSTransaction)txn).setApprover(digitalSaleSignature[1]);
												 
											// SALVO NEL ASISTRANSACTION LA STRINGA DA STAMPARE 
												  //  asisData.setTxnNo(digitalSaleSignature[2]);
												    ((CMSCompositePOSTransaction)txn).setASISTxnData(asisData);
											// SALVO NEL OGGETTO SERIALIZZATO IL TOTALE
												// theAppMgr.addGlobalObject("SALE_DIGITAL_SIGNATURE", digitalSaleSignature[0],true);
												 
										
												System.out.println("previousDigitalSignature IN SALE:"+previousSaleDigitalSignature);
												//System.out.println("ACTUAL DigitalSignature IN SALE :"+digitalSaleSignature[0]);						
								}
						  	}
					...
					//Sergio
					if (result == 0) {
						fiscalReceiptNumber = fiscalInterface.getDocumentResponse().getDocumentNumber(); //Sergio)
						//set Fiscal Number
						if (fiscalReceiptNumber != null && fiscalReceiptNumber.length() != 0) {
							CommonTransaction txn = (CommonTransaction) theAppMgr.getStateObject("TXN_POS");
							if (txn instanceof CMSCompositePOSTransaction) {
								theTxnPOS = (CMSCompositePOSTransaction) txn;
								theTxnPOS.setFiscalReceiptNumber(fiscalReceiptNumber); //Sergio not has to be saved but just after the printout
								theTxnPOS.setFiscalReceiptDate(fiscalInterface.getFiscalReceiptDate());
								
							} else if (txn instanceof CollectionTransaction) {
								((CMSCollectionTransaction) txn).setFiscalReceiptNumber(fiscalReceiptNumber);
								((CMSCollectionTransaction) txn).setFiscalReceiptDate(fiscalInterface.getFiscalReceiptDate());
							} else if (txn instanceof CMSPaidOutTransaction) {
								((CMSPaidOutTransaction) txn).setFiscalReceiptNumber(fiscalReceiptNumber);
								((CMSPaidOutTransaction) txn).setFiscalReceiptDate(fiscalInterface.getFiscalReceiptDate());
							}
						}
						fiscalNoLbl.setText(fiscalReceiptNumber);
						...
						//As per request dated : 08 october 2007 -WE need to modify the flow after an hardware error on the fiscal printer. 
					    System.out.println("MESSAGGIO USCITO");
					    LoggingServices.getCurrent().logMsg(getClass().getName(), "MESSAGE ERROR PAYMENT", "messaggio uscito", "messaggio uscito", LoggingServices.MAJOR);
						boolean doRetry = theAppMgr.showOptionDlg(res.getString("Print Fiscal Receipt Failed."),
								...
								res.getString("Retry"), res.getString("Post Txn"));
						if (doRetry) {
							//theAppMgr.unRegisterSingleEditArea();
							//theTxn.testIsSuspendable();
							//theAppMgr.showMenu(MenuConst.CANCEL_ONLY, theOpr);
              ...
              */
              int row = tblPayment.getRowCount();
					    ...
					    Payment payment = (Payment)vTemp.elementAt(i);
							...
							System.out.println("tableRows count in table------->"+tableRows);
...
//				} catch (BusinessRuleException bre) {
//					theAppMgr.showErrorDlg(bre.getMessage());
//					//          theAppMgr.showMenu(MenuConst.POST, "PRINT_FISCAL_RECEIPT", theOpr);
//					//          PaymentApplet_EUR.this.appButtonEvent(new CMSActionEvent(this, 0, "PRINT_FISCAL_RECEIPT"
//					//              , 0));
//					//          return;
				} catch (Exception ex) {
					ex.printStackTrace();
				...
				if (isButtonInList(sAction)) {
					...
					if (!flag) {
						...
						if (!bool) {
							...
							//if the customer is already present when coming to PaymentApplet_EUR for the
							//first time
							if (flag) {
								theAppMgr.addStateObject("PAYMENT", sAction);
								theAppMgr.fireButtonEvent("CUST_SALE_HIDDEN");
	...
	void tblPayment_componentResized(ComponentEvent e) {
		...
		tblPayment.getColumnModel().getColumn(model.AMOUNT).setPreferredWidth(175);
		//End
	...
	public boolean isHomeAllowed() {
		...
		if (!isPostComplete && !cancelTxnPressed) {
			return (theAppMgr.showOptionDlg(res.getString("Cancel Transaction"), res.getString("Selecting 'Home' cancels the transaction.  Do you want to cancel this transaction?")));
		} else {
	...
	/**
	 * To be used to set the font of "Amount Due" and "Amount Tendered" in PaymentApplet_EUR
	...
	public void fireSetefiResult(Object p) {
		...
		if (sr.getRespStatusCode().compareTo("01") == 0) {
			...
			System.out.print(sr.getRespStatusCode() + "\n"); //Status code 01=OK 02=Not approved 03=Referral 04=timeout 05 = systemerror
			...
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					theAppMgr.unRegisterSingleEditArea();
					theAppMgr.buildObject("PAYMENT", CMSPaymentMgr.getPaymentBuilder("CREDIT_CARD"), PaymentApplet_EUR.this.creditCardAmountForSetefi);
				}
			});
			//                     return;
		}
	...
	private CMSPaymentCode getPaymentCode(String paymentType) {
		...
		if (numPaymentCodes == 1) {
			...
			if (overRideDlg.isOK()) {
				return (CMSPaymentCode) overRideDlg.getSelectedRow().getRowKeyData();
	...
	/*
	 * Method for PCR from Europe 1864
	 * For VAT Exempt Transaction : After the ticket printout all the buttons pressed by the operator except "Fiscal Document" will produce
	 * a warning window with "Mandatory add a fiscal Document on this transaction". 
	 * 
	 * 
	 */
	...
	private class PaymentRenderer extends JLabel implements TableCellRenderer {
		...
		public PaymentRenderer() {
			...
			setBackground(theAppMgr.getTheme().getTableSelectionBackgroundColor());
