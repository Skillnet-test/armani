...
public class ItemBldr implements IObjectBuilder {
        ...
        if (item.getRedeemableType().equals(Redeemable.STORE_VALUE_CARD_ADD_TYPE)) {
          if (CMSRedeemableHelper.findStoreValueCard(theAppMgr, sCtrlNo) == null) {
            theAppMgr.showErrorDlg(applet.res.getString(
                "There is not a stored value card on file with that control number."));
            completeAttributes();
            return;
          }
        } else if (!CMSRedeemableHelper.isNewGiftCertControlNumberValid(theAppMgr, sCtrlNo)) {
          theAppMgr.showErrorDlg(applet.res.getString(
              "That is not a valid control number or it may already be in use."));
          completeAttributes();
          return;
          ...
          for (; lineItems.hasMoreElements(); ) {
            ...
            for (; details.hasMoreElements(); ) {
              ...
              if (lineItemDetail.getGiftCertificateId() != null
                  && lineItemDetail.getGiftCertificateId().length() > 0
                  && lineItemDetail.getGiftCertificateId().equals(sCtrlNo)) {
                theAppMgr.showErrorDlg(applet.res.getString(
                    "This transaction already has an item with that control ID."));
                completeAttributes();
                return;
              }
            }
      ...
      try {
        ...
        if (key.length() > 2) {
          ...
          if (itemIdArray == null || itemIdArray.length == 0) {
            theAppMgr.showErrorDlg(applet.res.getString("No items found matching your description."));
            theBldrMgr.processObject(applet, "ITEM", null, this);
            return;
          } else {
          ...
          theAppMgr.showErrorDlg(applet.res.getString(
              "The systems requires at least three letters to search for."));
        } else {
          theBldrMgr.processObject(applet, "ITEM", null, this);
          return;
        }
  ...
  private void build(String Command, CMSApplet applet, MiscItemTemplate initValue) {
    ...
    try {
      ...
      if (openDepositTypes.contains(miscItemTemplate.getKey())
        ...
        CMSCustomer customer = (CMSCustomer)((CMSCompositePOSTransaction)theAppMgr.getStateObject(
            "TXN_POS")).getCustomer();
        if (customer == null) {
  ...
  public void build(String Command, CMSApplet applet, Object initValue) {
    ...
    if (MiscItemManager.getInstance().isMiscItem(initValue.toString())) {
      theAppMgr.showErrorDlg(applet.res.getString(
          "Non-merchandise items must be entered using the Services Menu"));
    ...
    if (((String)initValue).equalsIgnoreCase("S")) {
      ...
      try {
        ...
        //code added by Anand on 04/08/2005 to enable search based on retail barcode(which is returned by
        // the getRetailBarcode() of ItemEntryParser, rather than the entire barode that is scanned/keyed in
        ItemEntryParser itmParser = new ItemEntryParser();
        String longerBarCode  = null;
        boolean flag = true;
        String retailBarcode = itmParser.getRetailBarcode(applet, theAppMgr, sItemValue
            , new Integer(sItemValue.length()).toString());
       
 // Modified by Anjana to fetch the proper barcode based on SAP or Retek Store
        ...
        String hasSapLookup = mgr.getString("SAP_LOOKUP");
			if (retailBarcode != null) {
				...
				if (retailBarcode.length() > 8 && retailBarcode.length() <= 18) {
				theAppMgr.unRegisterSingleEditArea();
				theAppMgr.getMainFrame().getGlobalBar().setTextAreaEnabled(false);
				item = CMSItemHelper.findSAPItem(theAppMgr, retailBarcode, storeId);
					}
				} else {
						theAppMgr.unRegisterSingleEditArea();
						theAppMgr.getMainFrame().getGlobalBar().setTextAreaEnabled(false);
						item = CMSItemHelper.findByBarCode(theAppMgr,retailBarcode, storeId);
					}//End Anjana's changes to fetch the proper barcode based on SAP or Retek Store
          //1864
          itemWrapper = new CMSItemWrapper();
          itemWrapper.setItem(item);
          itemWrapper.setExtendedBarCode(extendedBarCode);
  ...
  private boolean solicitItemDescription() {
    ...
    for (int i = 0; i < descriptions.length; i++) {
      availDescriptions[i] = new GenericChooserRow(new String[] {descriptions[i]
      }
          , descriptions[i]);
    }
    GenericChooseFromTableDlg dlg = new GenericChooseFromTableDlg(theAppMgr.getParentFrame()
        , theAppMgr, availDescriptions, new String[] {(applet.res.getString("Item Description"))
    });
    ...
    if (dlg.isOK()) {
      ...
      theAppMgr.showErrorDlg(applet.res.getString(
          "You did not select a description, you must indicate a description."));
      return (false);
    }
  }
  ...
  private boolean solicitMiscItemTemplateChoice() {
    MiscItemTemplate[] miscItemTemplates = MiscItemManager.getInstance().getMiscItemsArray(item.
        getId());
    ...
    for (int i = 0; i < availMiscItemTemplates.length; i++) {
      availMiscItemTemplates[i] = new GenericChooserRow(new String[] {miscItemTemplates[i].
          getMiscItemDescription()
      }
          , miscItemTemplates[i]);
    }
    GenericChooseFromTableDlg dlg = new GenericChooseFromTableDlg(theAppMgr.getParentFrame()
        , theAppMgr, availMiscItemTemplates
        , new String[] {(applet.res.getString("Miscellaneous Item"))
    });
    dlg.getTable().getColumnModel().getColumn(0).setCellRenderer(centerRenderer);
    dlg.setVisible(true);
    if (dlg.isOK()) {
      miscItemTemplate = (MiscItemTemplate)((MiscItemTemplate)dlg.getSelectedRow().getRowKeyData()).
          clone();
      try {
        miscItem = new CMSMiscItem(miscItemTemplate.getKey(), item);
        itemWrapper = new CMSItemWrapper();
      ...
      theAppMgr.showErrorDlg(applet.res.getString(
          "You did not select a miscellaneous item, item was not added."));
      return (false);
    }
  }
   ...
   */
  private void applyMiscItemTemplate()
    ...
    if (!miscItemTemplate.getCanOverrideDescription()) {
      String[] descriptions = miscItemTemplate.getDescription();
      if (descriptions != null && descriptions.length > 0 && miscItemTemplate.getDescIdx() > -1)
        miscItem.setDescription(descriptions[miscItemTemplate.getDescIdx()]);
    }
    ...
    miscItem.setDefaultQuantity(new Integer(miscItemTemplate.getDefaultQty()));
  }
  ...
  private boolean completeAttributes() {
    ...
    if (searchMode) {
      theAppMgr.showMenu(MenuConst.CANCEL_ONLY, "ITEMBUILDER", applet.theOpr);
      theAppMgr.setSingleEditArea(applet.res.getString("Enter key words from the item description.")
          , "SEARCH");
      return (false);
    }
    // no item was found
    ...
    if (miscItemTemplate != null && miscItemTemplate.getCanOverrideAmount()) {
      String pricePrompt = applet.res.getString("Enter price to charge for item.");
      if (openDepositTypes.contains(miscItemTemplate.getKey())
          || closeDepositTypes.contains(miscItemTemplate.getKey())) {
        if (custDepositBalance == null)custDepositBalance = new ArmCurrency(0.0d);
        if (openDepositTypes.contains(miscItemTemplate.getKey()))
      ...
      if (miscItemTemplate.getAmount() == null)
        theAppMgr.setSingleEditArea(pricePrompt, "MISC_PRICE", theAppMgr.CURRENCY_MASK);
      else
        theAppMgr.setSingleEditArea(pricePrompt, "MISC_PRICE", miscItemTemplate.getAmount()
            , theAppMgr.CURRENCY_MASK);
      return (false);
    }
    // misc item override description
    if (miscItemTemplate != null && miscItemTemplate.getCanOverrideDescription()) {
      if (solicitItemDescription()) {} else // and continue
        return (false);
    }
    // misc item override tax
    if (miscItemTemplate != null && miscItemTemplate.getCanOverrideTaxable()) {
      String initValue = miscItemTemplate.getTaxable() ? applet.res.getString("Y")
    ...
    if (miscItemTemplate != null && miscItemTemplate.getCanOverrideComment()) {
      if (miscItemTemplate.getComment() == null) {
        theAppMgr.setSingleEditArea(applet.res.getString("Enter comment for item."), "MISC_COMMENT"
            , theAppMgr.INPUT_IS_OPTIONAL_MASK);
      } else {
        // System.out.println("comment default: " + miscItemTemplate.getComment());
        theAppMgr.setSingleEditArea(applet.res.getString("Enter comment for item."), "MISC_COMMENT"
            , miscItemTemplate.getComment(), theAppMgr.INPUT_IS_OPTIONAL_MASK);
      }
      return (false);
    }
    ...
    if (item.isRedeemable()) {
      if (!theAppMgr.isOnLine()) {
        theAppMgr.showErrorDlg(
            "Redeemable items cannot be purchased or redeemed while in offline mode.");
        item = null;
        theBldrMgr.processObject(applet, "SPECIFIC", null, this);
        return (true); // there are no attributes to set on a null item.
      }
      if (applet instanceof InitialSaleApplet
        ...
        theAppMgr.showErrorDlg(
            "Redeemable items cannot be returned.  Use the 'Buy Back' option in the management menu.");
        item = null;
        return (true); // there are no attributes to set on a null item.
      }
      if (spcItem == null) {
      ...
      try {
        if (spcItem.getGiftCertificateId() == null || spcItem.getGiftCertificateId().length() == 0) {
          if (item.getRedeemableType().equals(Redeemable.GIFT_CERTIFICATE_TYPE)) {
            theAppMgr.setSingleEditArea(applet.res.getString(
                "Enter gift certificate ID or control number."), "GC_CONTROL_ID");
          } else { //if(item.getRedeemableType().equals(Redeemable.STORE_VALUE_CARD_TYPE))
            theAppMgr.setSingleEditArea(applet.res.getString(
                "Enter store value card ID or control number."), "GC_CONTROL_ID");
          }
          return (false);
        }
        if (spcItem.getUnitPrice() == null || spcItem.getUnitPrice().lessThan(new ArmCurrency(0.01))) {
          if (item.getRedeemableType().equals("G")) {
            theAppMgr.setSingleEditArea(applet.res.getString("Enter gift certificate amount.")
                , "GC_PRICE", theAppMgr.CURRENCY_MASK);
          } else { // if(item.getRedeemableType().equals("S"))
            theAppMgr.setSingleEditArea(applet.res.getString("Enter store value card amount.")
                , "GC_PRICE", theAppMgr.CURRENCY_MASK);
          }
          return (false);
        }
    ...
    // manual unit price
    if (miscItemTemplate == null && item.requiresManualUnitPrice()) { // || item.isRedeemable())  Quick fix, these switches are not independent but should be
      if (spcItem == null){
        spcItem = new CMSSpecificItem(item);
        itemWrapper = new CMSItemWrapper();
        itemWrapper.setSpecificItem(spcItem);
      ...
      if (spcItem.getUnitPrice() == null || spcItem.getUnitPrice().doubleValue() == 0) {
        theAppMgr.setSingleEditArea(applet.res.getString("Enter manual price amount."), "PRICE"
            , theAppMgr.CURRENCY_MASK);
        return (false);
      }
    }
...
//	  return (inputBarCode.length() == 34)?inputBarCode.substring(0, 11):inputBarCode;
	  return inputBarCode;
  }
  //Added for Dolci PCR vivek
  public void build(String Command, CMSApplet applet, String initValue,ArmCurrency amount) {
      try {
