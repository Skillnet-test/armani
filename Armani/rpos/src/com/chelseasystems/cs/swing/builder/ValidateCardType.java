...
public class ValidateCardType extends CreditCardBldrUtil{
	...
	public static Payment allocGiftCardPayment(String gfAccNo,IApplicationManager theAppMgr){
		...
		return received;
		/*CMSStoreValueCard cmsstorevaluecard=new CMSStoreValueCard("STORE_VALUE_CARD");
		cmsstorevaluecard.setId(gfAccNo);
		cmsstorevaluecard.setGUIPaymentName("Gift Card");
		cmsstorevaluecard.setTransactionPaymentName("STORE_VALUE_CARD");
		cmsstorevaluecard.setType("S");
		cmsstorevaluecard.doSetControlNum(gfAccNo);
		return cmsstorevaluecard;*/
		}catch(Exception e){
			  e.printStackTrace();
			  return null;
		}
...
//	    		||((codeJCB == 6500)&& (codeJCB ==6509))
	    ...
	    //     }
		      ...
		      return mc;
	       	...
	       	if(payDesc.equalsIgnoreCase("JCB") || payDesc.equalsIgnoreCase("DINERS") || payDesc.equalsIgnoreCase("CUP") || payDesc.equalsIgnoreCase("DISCOVER")){
	       	JCBCreditCard jcb = new JCBCreditCard(CMSIPaymentConstants.JCB);
    		accountNum = firstSixOrEight + "****" + lastFour;
 	        jcb.setAccountNumber(accountNum);
 	        jcb.setMaskCardNum(accountNum);
