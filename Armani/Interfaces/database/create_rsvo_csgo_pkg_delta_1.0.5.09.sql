	...
	END LOOP;
  	   ...
  	   BEGIN
	 			FORALL i IN 1..txn_rec.record_type.COUNT()	SAVE EXCEPTIONS
					   ...
					   SELECT txn_rec.transaction_id(i) TRANSACTION_ID, txn_rec.sales_associate_id(i) SALES_ASSOCIATE_ID,
   ...
   (
	...
	BEGIN
		FOR i IN 1 .. txn_rec.record_type.COUNT LOOP
			l_pay_line_id := 0;
			BEGIN
				l_pay_line_id := pay_line_id(txn_rec.transaction_id(i))+1;
		 ...
		 BEGIN
	 			FORALL i IN 1..txn_rec.record_type.COUNT()	SAVE EXCEPTIONS
					   ...
					   SELECT txn_rec.transaction_id(i) TRANSACTION_ID, arr_pay_line_id(i) line_id, txn_rec.tnd_type(i) TND_TYPE,
               					   Arm_Util_Pkg.convert_to_currency(txn_rec.tnd_amount(i), txn_rec.currency_code(i)) TND_AMOUNT  FROM DUAL;
	   		-- Exception section
		EXCEPTION
				WHEN DML_ERRORS THEN
					errors := SQL%BULK_EXCEPTIONS.COUNT;
