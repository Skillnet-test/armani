...
CREATE OR REPLACE PACKAGE BODY Arm_Load_Cust_Pkg IS
  ...
  STG_PROCESS_DATE
    ...
    BEGIN
        ...
        -- Update "ARM_CUST_DEPOSIT_HIST" table Begins --
        BEGIN
            FORALL i IN 1..customer_rec.record_type.COUNT()
            SAVE EXCEPTIONS
                UPDATE ARM_CUST_DEPOSIT_HIST
                SET CUST_ID=customer_rec.customer_id(i)
                WHERE CUST_ID=customer_rec.old_customer_id(i);
            -- Exception section
            EXCEPTION
                WHEN DML_ERRORS THEN
                    ...
                    END LOOP;
        END;
        -- Update "ARM_CUST_DEPOSIT_HIST"  table Ends --
        -- Update "ARM_CUST_CREDIT_HIST" table Begins --
        BEGIN
            FORALL i IN 1..customer_rec.record_type.COUNT()
            SAVE EXCEPTIONS
                UPDATE ARM_CUST_CREDIT_HIST
                SET CUST_ID=customer_rec.customer_id(i)
                WHERE CUST_ID=customer_rec.old_customer_id(i);
            -- Exception section
            EXCEPTION
                WHEN DML_ERRORS THEN
                    ...
                    END LOOP;
        END;
        -- Update "ARM_CUST_CREDIT_HIST"  table Ends --
