...
TYPE DEPOSIT_AMT_TYPE IS TABLE OF ARM_STG_RSVO_CSGO.DEPOSIT_AMT%TYPE;
...
CREATE OR REPLACE PACKAGE BODY Arm_Load_Rsvo_Csgo_Pkg IS
  ...
  BEGIN
  	   ...
  	   BEGIN
	 			FORALL i IN 1..txn_rec.record_type.COUNT()	SAVE EXCEPTIONS
					   ...
					   SELECT txn_rec.transaction_id(i) TRANSACTION_ID,	txn_rec.add_no(i) ADD_NO,
						...
						WHERE txn_rec.transaction_type(i) IN ('CSGO');
		-- Exception section
		EXCEPTION
				WHEN DML_ERRORS THEN
					errors := SQL%BULK_EXCEPTIONS.COUNT;
		...
		END;
  	   BEGIN
	 			FORALL i IN 1..txn_rec.record_type.COUNT()	SAVE EXCEPTIONS
					   INSERT INTO ARM_POS_RSV (AI_TRN, ID_RESERVATION,  EXP_DT, DEPOSIT_AMT)
					   SELECT txn_rec.transaction_id(i) TRANSACTION_ID,	txn_rec.add_no(i) ADD_NO,
					   		  			    txn_rec.expiration_date(i) EXPIRATION_DATE, txn_rec.deposit_amt(i) FROM DUAL
						WHERE txn_rec.transaction_type(i) IN ('RSVO');
		-- Exception section
		EXCEPTION
				WHEN DML_ERRORS THEN
					errors := SQL%BULK_EXCEPTIONS.COUNT;
					...
					FOR i IN 1..errors LOOP
						...
						UPDATE arm_stg_CUST_IN
						SET      stg_status = 1
						, stg_error_message = stg_error_message  || '|' || '  Error INSERTING	ARM_POS_RSV - ' || error_code
						, stg_process_date = SYSDATE
						WHERE  stg_event_id = txn_rec.stg_event_id(error_index);
					END LOOP;
		END;
	 			...
	 			FORALL i IN 1..txn_rec.record_type.COUNT()	SAVE EXCEPTIONS
					   ...
					   WHEN (1 = 1) THEN
					   		INTO RK_POS_LN_ITM_DTL (SEQUENCE_NUMBER,NET_AMT,TAX_AMT,MANUAL_TAX_AMT,
								 				    SALE_RETURNED,AI_TRN,AI_LN_ITM, FL_PROCESSED)
							VALUES (ROWNUM,LN_ITM_DTL_NET_AMT,LN_ITM_DTL_TAX_AMT,MANUAL_TAX_AMT,
								   '0', TRANSACTION_ID, LINE_ID, IS_PROCESSED)
					   WHEN (IS_PROCESSED IN ('1') AND LINE_ITEM_TYPE IN ('2')) THEN
					   		INTO ARM_RSV_POS_LN_ITM_DTL ( AI_TRN,  AI_LN_ITM, SEQUENCE_NUM,
								 						  		   			  			   		   		  ORG_AI_TRN, ORIG_AI_LN_ITM, ORIG_SEQ_NUM)
							VALUES(TRANSACTION_ID, LINE_ID, ROWNUM, TRANSACTION_ID, LINE_ID, ROWNUM)																			  		   
					   SELECT ROWNUM, txn_rec.is_process(i) IS_PROCESSED,
                                       ...
                                       txn_rec.transaction_id(i) TRANSACTION_ID, arr_line_id(i) LINE_ID, Arm_Util_Pkg.convert_to_currency(txn_rec.DEPOSIT_AMT(i), txn_rec.CURRENCY_CODE(i),'0') DEPOSIT_AMT,
									   txn_rec.line_item_type(i) line_item_type
