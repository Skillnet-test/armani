...
TYPE prm_exp_dt_type IS TABLE OF RK_PRM_ITM.dc_prm_ep%TYPE;
...
SELECT ID_ITM, BARCODE, ID_STR_RT, RETAIL_VALUE, MARKDOWN_VALUE,
 ...
 SC_ITM_SLS,  FL_EN_PRC_RQ, UPDATE_DT, ID_RU_PRDV, PRM_EXP_DT
FROM   RK_ITEM_VW
WHERE  ID_STR_RT = p_store_id
AND  ((UPDATE_DT >= p_from_date AND p_from_date IS NOT NULL) OR p_from_date IS NULL)
AND (NVL(QU_AVAILABLE,0)-NVL(QU_UNAVAILABLE,0))+(NVL(QU_STORE_AVAILABLE,0)-NVL(QU_STORE_UNAVAILABLE,0)) > 0
...
SQLERRM;
	...
	IF (last_item_id != current_item_id) THEN
		IF (trim(last_item_id) IS NOT NULL) THEN
	       ...
	       || '|' ;
		   IF (item_rec.id_ru_prdv(i) IS NOT NULL) THEN
               item_string := item_string || item_rec.id_ru_prdv(i)
			      || ',' ||
				  TO_CHAR(item_rec.prm_exp_dt(i),'YYYYMMDDHH24MISS');
			END IF;
    ...
    END LOOP;
	IF (trim(last_item_id) IS NOT NULL) THEN
		utl_file.put_line(output_file_handle, item_string);
	END IF;	
