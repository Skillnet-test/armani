...
--		 result := currency_cd||TRIM(TO_CHAR(ROUND(data, 6), number_format_mask));
		 result := currency_cd||TO_CHAR(ROUND(data, 6), nls_number_format, 'nls_numeric_characters='||nls_char||'''');
		 RETURN result;
	EXCEPTION
			 WHEN OTHERS THEN
			 		IF (fl_null = '0') THEN
...
--		 result := TO_NUMBER(REPLACE(data, currency_cd, ''),number_format_mask);
		 result := TO_NUMBER(REPLACE(data, currency_cd, ''), nls_number_format, 'nls_numeric_characters='||nls_char||'''');
		 RETURN result;
	EXCEPTION
			 WHEN OTHERS THEN
			 	  RETURN 0;
