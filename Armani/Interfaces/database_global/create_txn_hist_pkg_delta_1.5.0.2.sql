...
CREATE OR REPLACE PACKAGE BODY Arm_Load_Txn_Hist_Pkg IS
  ...
  BEGIN
		...
		END;
     BEGIN
	 FORALL i IN 1..txn_rec.record_type.COUNT
						UPDATE ARM_STG_TXN_HIST
                 SET STG_PROCESS_DATE=SYSDATE,
                 STG_STATUS=0
				WHERE  stg_event_id = txn_rec.stg_event_id(i)
			...
			BEGIN
				...
				EXCEPTION
						WHEN NO_DATA_FOUND THEN
							BEGIN
								SELECT NVL(MAX(AI_LN_ITM),0) INTO l_line_id FROM TR_LTM_RTL_TRN WHERE AI_TRN = txn_rec.transaction_id(i);
							EXCEPTION
								WHEN NO_DATA_FOUND THEN
	       ...
	       END;
			BEGIN
				...
				EXCEPTION
						WHEN NO_DATA_FOUND THEN
							BEGIN
								SELECT NVL(MAX(SEQ_NUM),0) INTO l_ship_seq_num FROM RK_SHIP_REQ WHERE AI_TRN = txn_rec.transaction_id(i);
							EXCEPTION
								WHEN NO_DATA_FOUND THEN
 	   ...
 	   BEGIN
	 			FORALL i IN 1..txn_rec.record_type.COUNT()	SAVE EXCEPTIONS
					   ...
					   SELECT txn_rec.RECORD_TYPE(i) RECORD_TYPE,	txn_rec.TRANSACTION_ID(i) TRANSACTION_ID,
					   		  			txn_rec.TRANSACTION_TYPE(i) TRANSACTION_TYPE, txn_rec.TRANSACTION_DATE(i) TRANSACTION_DATE,
		   		  						txn_rec.STORE_ID(i) STORE_ID, txn_rec.REGISTER_NO(i) REGISTER_NO,
										txn_rec.OPERATOR_ID(i) OPERATOR_ID,
										ASSOCIATE.ID_EM SALES_ASSOCIATE_ID,
										Arm_Util_Pkg.convert_to_currency(txn_rec.TRANSACTION_NET_AMOUNT(i), txn_rec.CURRENCY_CODE(i), '0') TRANSACTION_NET_AMOUNT,
										...
										Arm_Util_Pkg.convert_to_currency(txn_rec.MANUAL_MARKDOWN_AMT(i), txn_rec.CURRENCY_CODE(i), '1') MANUAL_MARKDOWN_AMT,
										Arm_Util_Pkg.convert_to_currency(txn_rec.MANUAL_UNIT_PRICE(i), txn_rec.CURRENCY_CODE(i), '1') MANUAL_UNIT_PRICE,
										Arm_Util_Pkg.convert_to_currency(txn_rec.LN_ITM_NET_AMT(i), txn_rec.CURRENCY_CODE(i),'0') LN_ITM_NET_AMT,
		...
		EXCEPTION
				WHEN DML_ERRORS THEN
					...
					FOR i IN 1..errors LOOP
						error_index := SQL%BULK_EXCEPTIONS(i).ERROR_INDEX;
						error_code := SQL%BULK_EXCEPTIONS(i).ERROR_CODE;
						error_text := SQLERRM(-SQL%BULK_EXCEPTIONS(i).ERROR_CODE) ;
						UPDATE ARM_STG_TXN_HIST
						SET      stg_status = 1
						, stg_error_message = stg_error_message  || '1:|' || '  Error INSERTING	TR_LTM_RTL_TRN - ' || error_code || error_text
						, stg_process_date = SYSDATE
						WHERE  stg_event_id = txn_rec.stg_event_id(error_index);
					END LOOP;
				WHEN OTHERS THEN
					...
					FOR i IN 1..errors LOOP
						...
						UPDATE ARM_STG_TXN_HIST
						SET      stg_status = 1
						, stg_error_message = stg_error_message  || '2:|' || '  Error INSERTING	TR_LTM_RTL_TRN - ' || error_code || error_text
						, stg_process_date = SYSDATE
						WHERE  stg_event_id = txn_rec.stg_event_id(error_index);
					END LOOP;
	   END;
		...
		EXCEPTION
				WHEN DML_ERRORS THEN
					...
					FOR i IN 1..errors LOOP
						...
						UPDATE ARM_STG_TXN_HIST
						SET      stg_status = 1
						, stg_error_message = stg_error_message  || '3:|' || '  Error UPDATING	TR_LTM_RTL_TRN - ' || error_code || error_text
						, stg_process_date = SYSDATE
						WHERE  stg_event_id = txn_rec.stg_event_id(error_index);
					END LOOP;
	   END;
  	   ...
  	   BEGIN
	 			FORALL i IN 1..txn_rec.record_type.COUNT()	SAVE EXCEPTIONS
					   INSERT ALL
							...
							WHEN (txn_rec.REDUCTION_REASON(i) IS NOT NULL) THEN	   
					   ...
					   SELECT txn_rec.transaction_id(i) TRANSACTION_ID, arr_line_id(i) LINE_ID,
					   		  			Arm_Util_Pkg.convert_to_currency(txn_rec.REDUCTION_AMT(i)/txn_rec.QUANTITY(i), txn_rec.CURRENCY_CODE(i), '0') REDUCTION_AMT,
		...
		EXCEPTION
				WHEN DML_ERRORS THEN
					...
					FOR i IN 1..errors LOOP
						...
						UPDATE ARM_STG_TXN_HIST
						SET      stg_status = 1
						, stg_error_message = stg_error_message  || '5:|' || '  Error INSERTING	RK_POS_LN_ITM_DTL/RK_REDUCTION - ' || error_code || error_text
						, stg_process_date = SYSDATE
						WHERE  stg_event_id = txn_rec.stg_event_id(error_index);
					END LOOP;
				WHEN OTHERS THEN
					...
					FOR i IN 1..errors LOOP
						...
						UPDATE ARM_STG_TXN_HIST
						SET      stg_status = 1
						, stg_error_message = stg_error_message  || '5:|' || '  Error INSERTING	RK_POS_LN_ITM_DTL/RK_REDUCTION - ' || error_code
						, stg_process_date = SYSDATE
						WHERE  stg_event_id = txn_rec.stg_event_id(error_index);
					END LOOP;
	 	 ...
	 	 BEGIN
		 	FORALL i IN 1..txn_rec.record_type.COUNT() SAVE EXCEPTIONS
		 	  INSERT ALL
				...
				VALUES(ADDR_ID, '1',  CUST_PHONE_1)
				INTO ARM_ADS_PH(ID_ADS, ID_PH_TYP,TL_PH)
				VALUES(ADDR_ID, '4',  CUST_PHONE_2)
	...
	BEGIN
		FOR i IN 1 .. txn_rec.record_type.COUNT LOOP
			...
			EXCEPTION
					WHEN NO_DATA_FOUND THEN
						BEGIN
							SELECT NVL(MAX(AI_LN_ITM),0) INTO l_pay_line_id FROM TR_LTM_TND WHERE AI_TRN = txn_rec.transaction_id(i);
		 ...
		 BEGIN
	 			FORALL i IN 1..txn_rec.record_type.COUNT()	SAVE EXCEPTIONS
					   ...
					   SELECT txn_rec.transaction_id(i) TRANSACTION_ID, arr_pay_line_id(i) line_id,
										...
										txn_rec.RDM_EXPIRATION_DATE(i) RDM_EXPIRATION_DATE, (SELECT COUNT(SEQ_NUM) FROM RK_REDM_HIST WHERE TY_GF_CF=txn_rec.RDM_TYPE(i) AND ID_NMB_SRZ_GF_CF=txn_rec.RDM_CONTROL_NUMBER(i)) SEQ_NUM  FROM DUAL;
	   	-- Exception section
		EXCEPTION
				WHEN DML_ERRORS THEN
					errors := SQL%BULK_EXCEPTIONS.COUNT;
					...
					FOR i IN 1..errors LOOP
						...
						UPDATE ARM_STG_TXN_HIST
						SET      stg_status = 1
						, stg_error_message = stg_error_message  || '|' || '  Error INSERTING TR_LTM_TND/TR_LTM_CR_DB_TN/TR_LTM_CHK_TND - ' || error_code || error_text
						, stg_process_date = SYSDATE
						WHERE  stg_event_id = txn_rec.stg_event_id(error_index);
					END LOOP;
				WHEN OTHERS THEN
		...
		EXCEPTION
				WHEN DML_ERRORS THEN
					...
					FOR i IN 1..errors LOOP
						...
						UPDATE ARM_STG_TXN_HIST
						SET      stg_status = 1
						, stg_error_message = stg_error_message  || '|' || '  Error UPDATING RK_PAY_TRN - ' || error_code || error_text
						, stg_process_date = SYSDATE
						WHERE  stg_event_id = txn_rec.stg_event_id(error_index);
					END LOOP;
		END;
