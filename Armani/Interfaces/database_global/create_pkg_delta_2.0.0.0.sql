...
CREATE OR REPLACE PACKAGE BODY Arm_Load_Data_Pkg IS
	...
	------------------------------------------------------------------------------------------------
		...
		WHERE ((NVL(STG_STATUS,0)= 0 AND STG_PROCESS_DATE IS NULL)
		...
		TYPE country_cd_type IS TABLE OF ARM_STG_CURRENCY_RT.country_cd%TYPE;
		TYPE lang_cd_type IS TABLE OF ARM_STG_CURRENCY_RT.lang_cd%TYPE;
		TYPE fin_cd_type IS TABLE OF ARM_STG_CURRENCY_RT.fin_cd%TYPE;
		...
		TYPE currency_rec_type IS RECORD (
			...
			, country_cd country_cd_type
			, lang_cd lang_cd_type
			, fin_cd fin_cd_type
	...
	BEGIN
	    OPEN  currency_cur;
		    ...
		    INTO    currency_rec.from_currency
				  ...
				  , currency_rec.country_cd
				  , currency_rec.lang_cd
				  , currency_rec.fin_cd
		...
		-- Loading Currency Rate Data
		BEGIN
			FORALL i IN 1..currency_rec.from_currency.COUNT() SAVE EXCEPTIONS
				MERGE INTO ARM_CURRENCY_RATE DESTINATION
				USING (SELECT   currency_rec.from_currency(i) from_currency
													 , currency_rec.to_currency(i) to_currency
													 , currency_rec.currency_rate(i) currency_rate
													 , TO_DATE(currency_rec.update_date(i),'YYYYMMDD') update_date
													, currency_rec.country_cd(i) country_cd
													, currency_rec.lang_cd(i) lang_cd
													, currency_rec.fin_cd(i) fin_cd
				FROM ARM_STG_CURRENCY_RT
				WHERE ((NVL(STG_STATUS,0)= 0 AND STG_PROCESS_DATE IS NULL)
				OR (STG_STATUS=2 AND STG_PROCESS_DATE IS NOT NULL))) SOURCE
				ON (DESTINATION.id_cny_from=SOURCE.from_currency
				AND DESTINATION.id_cny_to=SOURCE.to_currency
				AND DESTINATION.ED_CO=SOURCE.COUNTRY_CD
				AND DESTINATION.ED_LA=SOURCE.LANG_CD)
				WHEN MATCHED THEN
					UPDATE SET mo_rt_to_buy=SOURCE.currency_rate,
					update_dt=SOURCE.update_date,
					cd_fin=SOURCE.fin_cd
		...
		-- Update the Process Date of the Processed Record
	    BEGIN
	        UPDATE arm_stg_currency_rt
		...
		-- Logging an entry in the Process Log Table
		BEGIN
	...
	TYPE stg_event_id_type IS TABLE OF ARM_STG_REGISTER.stg_event_id%TYPE;
	...
	BEGIN
	    ...
	    FORALL i IN 1..register_rec.company_cd.COUNT()
	        ...
	        WHEN OTHERS THEN
	            ...
	            return_message := SQLERRM;
	    END;
	    IF ( NVL(return_status,'A') != l_failure ) THEN
	...
	BEGIN
		...
		-- Total No. Of Records Processed		
	    BEGIN
	        SELECT COUNT(*) INTO l_num_records_processed FROM ARM_STG_TAX_RATE WHERE (NVL(STG_STATUS,0)= 0 AND STG_PROCESS_DATE IS NULL)
	        OR (STG_STATUS=2 AND STG_PROCESS_DATE IS NOT NULL);
	    EXCEPTION
		...
		-- Merging of Tax Rate Data	
	    BEGIN
	        MERGE INTO ARM_TAX_RATE DESTINATION
	        USING (SELECT ZIP_CODE, CITY, COUNTY, STATE, TAX_RATE, EFFECTIVE_DT
	        FROM ARM_STG_TAX_RATE WHERE ((NVL(STG_STATUS,0)= 0 AND STG_PROCESS_DATE IS NULL)
	        OR (STG_STATUS=2 AND STG_PROCESS_DATE IS NOT NULL))) SOURCE
		...
		-- Deleting the old historical tax rate data
	    BEGIN
	...
	BEGIN
	    OPEN INVENTORY_cur;
	        LOOP
				...
				-- Merging the Inventory Information
	            BEGIN
	              FORALL i IN 1..inventory_rec.id_itm.COUNT SAVE EXCEPTIONS
	                ...
	                WHEN MATCHED THEN
	                    UPDATE SET BALANCE_DATE=source.balance_date,
	                        ...
	                        QU_STORE_AVAILABLE=DECODE(source.fl_balanced, '1', Arm_Get_Item_Soh(source.balance_date,SOURCE.STORE_ID, SOURCE.ITEM_ID), QU_STORE_AVAILABLE),
	...
	BEGIN
	    OPEN REDEEMABLE_cur;
	        LOOP
				...
				-- Merging the Redeemable Account Information
	            BEGIN
	            ...
	            EXCEPTION
	                WHEN dml_errors THEN
	                    ...
	                    END LOOP;
	            END;
				-- Inserting the Redeemable History Data
	            BEGIN
	                FORALL i IN 1..redeemable_rec.CONTROL_ID.COUNT SAVE EXCEPTIONS
	...
	BEGIN
		...
		-- Merging of Tax Free Data
	    BEGIN
	        MERGE INTO ARM_TAX_FREE DESTINATION
	        USING 
			(SELECT STORE_CODE, DETAX_CODE, DESC_CODE, 
								RATIO, START_DATE, NET_MIN_VALUE,
								GROSS_MIN_VALUE, PAYMENT_CODE, 
								MIN_PRICE, MAX_PRICE, MIN_AMOUNT, 
								MAX_AMOUNT, REFUND_AMOUNT, 
								REFUND_PERCENT, COMMISSION,
								PA_STR_RTL.TY_CNY CURRENCY_CD
	        FROM ARM_STG_TAX_FREE INNER JOIN PA_STR_RTL ON ARM_STG_TAX_FREE.STORE_CODE = PA_STR_RTL.ID_STR_RT 
	        ...
	        WHEN MATCHED THEN
	            UPDATE SET DESC_CODE=SOURCE.DESC_CODE,
								            RATIO=SOURCE.RATIO,
								            NET_MIN_VALUE=SOURCE.CURRENCY_CD||SOURCE.NET_MIN_VALUE,
								            GROSS_MIN_VALUE=SOURCE.CURRENCY_CD||SOURCE.GROSS_MIN_VALUE,
								            PAYMENT_CODE=SOURCE.PAYMENT_CODE,
								            MIN_PRICE=SOURCE.CURRENCY_CD||SOURCE.MIN_PRICE,
								            MAX_PRICE=SOURCE.CURRENCY_CD||SOURCE.MAX_PRICE,
								            MIN_AMOUNT=SOURCE.CURRENCY_CD||SOURCE.MIN_AMOUNT,
								            MAX_AMOUNT=SOURCE.CURRENCY_CD||SOURCE.MAX_AMOUNT,
								            REFUND_AMOUNT=SOURCE.CURRENCY_CD||SOURCE.REFUND_AMOUNT,
	        ...
	        WHEN NOT MATCHED THEN
	            ...
	            VALUES (SOURCE.STORE_CODE, SOURCE.DETAX_CODE, SOURCE.DESC_CODE, SOURCE.RATIO,
						            SOURCE.START_DATE, SOURCE.CURRENCY_CD||SOURCE.NET_MIN_VALUE, SOURCE.CURRENCY_CD||SOURCE.GROSS_MIN_VALUE, 
									SOURCE.PAYMENT_CODE, SOURCE.CURRENCY_CD||SOURCE.MIN_PRICE, SOURCE.CURRENCY_CD||SOURCE.MAX_PRICE, 
									SOURCE.CURRENCY_CD||SOURCE.MIN_AMOUNT, SOURCE.CURRENCY_CD||SOURCE.MAX_AMOUNT,
						            SOURCE.CURRENCY_CD||SOURCE.REFUND_AMOUNT, SOURCE.REFUND_PERCENT, SOURCE.COMMISSION);
	    EXCEPTION
	        WHEN OTHERS THEN
	            l_return_status := '1';
	            l_return_message := return_message || ',' || SQLERRM;
		...
		-- Deleting the Old Record based on the start date
	    BEGIN
	...
	BEGIN
		...
		-- Merging Fiscal Document Number data
	    BEGIN
	        MERGE INTO ARM_FISCAL_DOC_NO DESTINATION
	        USING 
			(SELECT REGISTER_ID, ID_STR_RT, LAST_DDT_NO, 
								 LAST_VAT_NO, LAST_CREDIT_NOTE
	        ...
	        WHEN NOT MATCHED THEN
	            INSERT (REGISTER_ID, ID_STR_RT, LAST_DDT_NO, 
					   				 LAST_VAT_NO, LAST_CREDIT_NOTE)
	            VALUES  (SOURCE.REGISTER_ID, SOURCE.ID_STR_RT, 
									 SOURCE.LAST_DDT_NO, SOURCE.LAST_VAT_NO,
