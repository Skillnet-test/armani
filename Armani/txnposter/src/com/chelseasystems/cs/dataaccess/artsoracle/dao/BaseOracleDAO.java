...
public abstract class BaseOracleDAO
  ...
  public void executeCaseSensitive (ParametricStatement sql) throws SQLException {
    try {
      ...
      se.printStackTrace();
      if( !(se instanceof CMSSQLException))
        throw  new CMSSQLException(se, sql);
    }
  }
      ...
      try {
        ...
        for (int i = 0; i < statements.length; i++) {
          try {
            ...
            if( !(sqlExp instanceof CMSSQLException))
              throw  new CMSSQLException(sqlExp, statements[i]);
          }
        }
        connection.commit();
      } catch (SQLException se) {
  ...
  public BaseOracleBean[] query (BaseOracleBean bean, String sql, List params) throws SQLException {
    ...
    try {
      ...
      try {
        ...
        if( !(sqlex instanceof CMSSQLException))
          throw  new CMSSQLException(sqlex, parametricStatement);
      }
    } finally {
      if (pool != null)
  ...
  public String[] queryForIds (String sql, List params) throws SQLException {
    ...
    try {
      ...
      try {
        ...
        exp.printStackTrace();
        pool.releaseConnection(connection);
        if( !(exp instanceof CMSSQLException))
          throw  new CMSSQLException(exp, parametricStatement);
      }
    } finally {
      pool.releaseConnection(connection);
    }
